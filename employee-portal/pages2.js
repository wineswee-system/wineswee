...
function openExpenseModal() {
  openModal(`
    ...
    <div class="form-actions">
      ...
      <button class="btn btn-primary" onclick="handleExpenseSubmit()">送出申請</button>
    </div>
  `);
}
async function handleExpenseSubmit() {
  ...
  const form = { category: select?.value, amount: numInput?.value, date: dateInput?.value, description: textarea?.value || '' };
  closeModal();
  if (isApiAvailable()) {
    const ok = await submitExpenseRequest(form);
    showToast(ok ? '費用申請已送出並建立簽核流程！' : '費用申請送出失敗', ok ? 'check-circle-2' : 'alert-circle');
  } else {
    showToast('費用申請已送出！（Demo 模式）', 'check-circle-2');
  }
}
...
function openBizTripModal() {
  openModal(`
    ...
    <div class="form-actions">
      <button class="btn btn-ghost" onclick="closeModal()">取消</button>
      <button class="btn btn-primary" onclick="handleBizTripSubmit()">送出申請</button>
    </div>
  `);
}
async function handleBizTripSubmit() {
  const modal = document.getElementById('modalContent');
  const textInput = modal.querySelector('input[type=text]');
  const dateInputs = modal.querySelectorAll('input[type=date]');
  const textarea = modal.querySelector('textarea');
  const numInput = modal.querySelector('input[type=number]');
  const form = { destination: textInput?.value, startDate: dateInputs[0]?.value, endDate: dateInputs[1]?.value, purpose: textarea?.value || '', estimatedAmount: numInput?.value || 0 };
  closeModal();
  if (isApiAvailable()) {
    const ok = await submitBusinessTrip(form);
    showToast(ok ? '出差申請已送出並建立簽核流程！' : '出差申請送出失敗', ok ? 'check-circle-2' : 'alert-circle');
  } else {
    showToast('出差申請已送出！（Demo 模式）', 'check-circle-2');
  }
}
