// 104 五月排班 → bulk_import_schedule migration 產生器
...
const F_SCHED = 'C:/Users/user/Downloads/20260714排班總表.xlsx'
...
async function main() {
  ...
  for (const row of rows) {
    ...
    for (let i = 0; i < dateCols.length; i++) {
      ...
      const base = { employee_id: e.id, employee: e.name, organization_id: e.org, date, shift: cell, store_id: e.store_id, source_store: e.store, month_group: '2026-05', status: 'draft' }
  ...
  const out = []
  out.push('-- 104 五月排班匯入 — auto-generated（來源：20260714排班總表 + 104班別）')
  ...
  out.push('')
  fs.writeFileSync(path.join(ROOT, 'supabase/migrations/20260714160000_import_104_may_schedule.sql'), out.join('\n'), 'utf8')

  console.log('✓ 產出 20260714160000_import_104_may_schedule.sql')
