...
export const KNOWLEDGE_BASE = [
  ...
  },
  {
    ...
    title: '費用報銷',
    path: '/hr/expenses',
    keywords: ['核銷', '報帳', '報銷', '費用', 'expense reimbursement'],
    steps: [
      '進入「行政庶務 → 費用報銷」',
      '上傳發票、填入金額與科目',
      '送出後依「費用簽核設定」自動派簽',
  ...
  // 2026-05-13：各表單頁的「⚙ 簽核設定」改為 navigate 到 /process/settings/chains/edit 頁面
  //   （不再用 modal）。FormBuilder 內仍保留 modal（因為它本身就是表單模板編輯 modal）。
  {
    ...
    steps: [
      ...
      '申請費用 (兩階段) 為例外：在 /hr/expense-requests 頁兩個按鈕分別設定「申請」「核銷」金額分組',
