...
export default function BusinessTravel() {
  ...
  const load = () => {
    ...
    return Promise.all([
      ...
      supabase.from('employees').select('id, name, dept, department_id, position, signature_url, departments!department_id(name)').eq('status', '在職').not('is_archived', 'is', true).eq('organization_id', orgId).order('name'),
