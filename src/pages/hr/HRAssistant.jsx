...
export default function HRAssistant() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase.from('employees').select('id, name, dept, store, department_id, position, store_id, join_date, status, departments!department_id(name), stores!store_id(name)').eq('status', '在職').not('is_archived', 'is', true).order('name'),
