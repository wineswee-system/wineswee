...
export default function AuditReport() {
  ...
  const exportXlsx = async () => {
    ...
    if (cats.length) {
      ...
      const h3 = ['分類', '次數', '涉及門市', '扣分', '', '']
      rows.push(h3);['分類', '次數', '涉及門市', '扣分'].forEach((_, c) => put(R, c, { font: { bold: true, sz: 11, color: { rgb: 'FFFFFF' } }, fill: { fgColor: { rgb: WINE } }, alignment: { horizontal: c === 0 ? 'left' : 'center', vertical: 'center' }, border })); R++
      cats.forEach((c, i) => {
        rows.push([c.name, c.cnt, (c.stores === scores.length && scores.length > 0) ? `${c.stores} 家全中` : `${c.stores} 家`, c.deduct, '', ''])
        const zeb = i % 2 ? { fgColor: { rgb: ZEBRA } } : undefined
        ...
        <div className="ar-sheet">
          ...
          {cats.length > 0 && (
            <div className="ar-card">
              ...
              <div style={{ overflowX: 'auto' }}>
                <table>
                  <thead><tr><th className="l">分類</th><th style={{ width: 64 }}>次數</th><th style={{ width: 100 }}>涉及門市</th><th className="l" style={{ width: 220 }}>扣分</th></tr></thead>
                  <tbody>
                    {cats.map(c => (
                      <tr key={c.name}>
                        <td className="ar-l ar-store" style={{ fontSize: 14 }}>{c.name}</td>
                        <td className="ar-c ar-revi">{c.cnt}</td>
                        <td className="ar-c">{c.stores === scores.length && scores.length > 0
