...
export default function Tasks() {
  ...
  const refresh = () => {
    ...
    return Promise.all([
      getTasks({ orgId }),
      supabase.from('employees').select('id, name, department_id, position, dept').eq('status', '在職').not('is_archived', 'is', true).eq('organization_id', orgId).order('name'),
  ...
  const handleSubmit = async () => {
    ...
    if (data) {
      ...
      // 綁定表單 → 建 binding（自己填/他人填+指派）+ 自己填暫存落地 + 取得建立後待跳出佇列
      const q = await createTaskBindings(data.id, form.required_forms, profile, {
  ...
  return (
    <div className="fade-in">
      ...
      {selfFillQueue && (
        <SelfFillQueue
          bindings={selfFillQueue.bindings}
          allBindings={selfFillQueue.all}
          onDone={() => setSelfFillQueue(null)}
        />
      )}
    </div>
  )
