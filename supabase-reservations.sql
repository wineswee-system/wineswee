...
-- ─── Reservation Rules ────────────────────────────────────────
-- Per-store, per-day-of-week booking policy.
-- Priority: date_override > day_of_week match > default (both NULL)
CREATE TABLE IF NOT EXISTS reservation_rules (
  id                    UUID         PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id       UUID         NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  store_id              UUID         NOT NULL REFERENCES stores(id) ON DELETE CASCADE,
  day_of_week           SMALLINT     CHECK (day_of_week BETWEEN 0 AND 6),  -- 0=Sun 6=Sat; NULL=default
  date_override         DATE,                                               -- specific date overrides day_of_week
  open_time             TIME         NOT NULL DEFAULT '10:00',
  close_time            TIME         NOT NULL DEFAULT '22:00',
  slot_interval_minutes INT          NOT NULL DEFAULT 60,   -- 60 = slots at 10:00, 11:00 …
  buffer_minutes        INT          NOT NULL DEFAULT 30,   -- table turn time after session ends
  end_buffer_minutes    INT          NOT NULL DEFAULT 30,   -- hard cutoff before close
...
-- ─── RLS: Authenticated staff ─────────────────────────────────
CREATE POLICY "org members manage reservation_rules"
  ON reservation_rules FOR ALL TO authenticated
  USING (organization_id IN (
    SELECT organization_id FROM employees WHERE auth_user_id = auth.uid()
  ));

CREATE POLICY "org members manage res_tables"
  ON res_tables FOR ALL TO authenticated
  USING (organization_id IN (
    SELECT organization_id FROM employees WHERE auth_user_id = auth.uid()
  ));

CREATE POLICY "org members manage table_combinations"
  ON table_combinations FOR ALL TO authenticated
  USING (organization_id IN (
    SELECT organization_id FROM employees WHERE auth_user_id = auth.uid()
  ));

CREATE POLICY "org members manage reservations"
  ON reservations FOR ALL TO authenticated
  USING (organization_id IN (
    SELECT organization_id FROM employees WHERE auth_user_id = auth.uid()
  ));

-- ─── RLS: Anonymous guests (public booking app) ───────────────
CREATE POLICY "anon create reservations"
  ON reservations FOR INSERT TO anon
  WITH CHECK (true);

CREATE POLICY "anon view reservations"
  ON reservations FOR SELECT TO anon
  USING (true);

CREATE POLICY "anon cancel reservation"
  ON reservations FOR UPDATE TO anon
  USING (true)
  WITH CHECK (status = 'cancelled');

CREATE POLICY "anon read res_tables"
  ON res_tables FOR SELECT TO anon
  USING (is_active = true);

CREATE POLICY "anon read reservation_rules"
  ON reservation_rules FOR SELECT TO anon
  USING (true);
...
BEGIN
  ...
  IF p_duration_hours < v_rule.min_booking_hours
    OR p_duration_hours > v_rule.max_booking_hours            THEN RETURN; END IF;

  v_last_allowed := v_rule.close_time
    - make_interval(mins => p_duration_hours * 60 + v_rule.end_buffer_minutes);

  v_slot := v_rule.open_time;

  WHILE v_slot <= v_last_allowed LOOP
    ...
    WHERE t.store_id  = p_store_id
      ...
      AND NOT EXISTS (
        ...
        WHERE r.table_id      = t.id
          ...
          AND r.slot_time < (v_slot_end + make_interval(mins => v_rule.buffer_minutes))
          AND (r.slot_time + make_interval(mins => (r.duration_hours + r.extended_hours) * 60)) > v_slot
      );
...
CREATE INDEX IF NOT EXISTS idx_res_changelog_rsv
  ON reservation_changelogs(reservation_id, created_at DESC);

ALTER TABLE reservation_changelogs ENABLE ROW LEVEL SECURITY;

-- Staff can read changelogs for their org's reservations
CREATE POLICY "org members read changelogs"
  ON reservation_changelogs FOR SELECT TO authenticated
  USING (
    reservation_id IN (
...
-- Staff insert (with employee_id)
CREATE POLICY "authenticated insert changelogs"
  ON reservation_changelogs FOR INSERT TO authenticated
  WITH CHECK (true);

-- Patch note: run this if reservations table was already created without created_by
-- ALTER TABLE reservations ADD COLUMN IF NOT EXISTS created_by UUID REFERENCES employees(id);
