...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns
    ...
    -- Removed static SQL queries that fail compilation when workflow_steps is missing columns
    RAISE NOTICE 'Skipping workflow_steps data migration as table or columns are not present';
