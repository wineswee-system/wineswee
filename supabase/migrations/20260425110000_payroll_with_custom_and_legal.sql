-- ============================================================
-- 升級 generate_payroll()：把自訂津貼 + 法扣納入結算
--
-- 變動：
--   1. payroll_records 加 3 個欄位儲存自訂津貼 + 法扣明細
--   2. generate_payroll() 函數：
--      a. 從 salary_structures.custom_allowances 累加金額到 v_gross
--      b. 計算完一般扣項後，依序處理 legal_deductions：
--         - 每筆「應扣 = min(monthly_amount, total - paid, 可動用淨額)」
--         - 真扣到 paid_amount，若達 total_amount 自動標記「已完成」
--         - 把該月扣款明細寫入 payroll_records.legal_deduction_breakdown
--      c. 若所有應扣加起來超過淨額 → 該月扣多少算多少，未扣完延後
--
-- 重要：法扣會更新 legal_deductions 表，所以「跑薪資」跟「修改法扣」會互相影響。
--       建議廠商：先檢查法扣設定 → 再跑薪資 → 不要事後改月份的法扣金額。
-- ============================================================

-- ═══ 1. payroll_records 加欄位 ═══
ALTER TABLE public.payroll_records
  ADD COLUMN IF NOT EXISTS custom_allowances_total NUMERIC(10,2) DEFAULT 0;
ALTER TABLE public.payroll_records
  ADD COLUMN IF NOT EXISTS custom_allowances_breakdown JSONB DEFAULT '[]'::jsonb;
ALTER TABLE public.payroll_records
  ADD COLUMN IF NOT EXISTS legal_deduction_total NUMERIC(10,2) DEFAULT 0;
ALTER TABLE public.payroll_records
  ADD COLUMN IF NOT EXISTS legal_deduction_breakdown JSONB DEFAULT '[]'::jsonb;

COMMENT ON COLUMN public.payroll_records.custom_allowances_breakdown IS
  '結算當下的自訂津貼快照 [{"name":"夜班津貼","amount":3000}, ...]';
COMMENT ON COLUMN public.payroll_records.legal_deduction_breakdown IS
  '此月實際扣的法扣明細 [{"id":1,"title":"養育費","amount":10000,"shortfall":0}, ...]';


-- ═══ 2. 升級 generate_payroll ═══
...
-- ═══ 3. LIFF RPC：員工查薪水時帶出自訂津貼 + 法扣 ═══
CREATE OR REPLACE FUNCTION public.liff_get_my_payroll_records(
  p_line_user_id text,
  ...
  RETURN json_build_object(
    ...
    'records', (
      SELECT COALESCE(json_agg(json_build_object(
        ...
        'role_allowance',              pr.role_allowance,
        'meal_allowance',              pr.meal_allowance,
        'transport_allowance',         pr.transport_allowance,
        'attendance_bonus_earned',     pr.attendance_bonus_earned,
        'overtime_pay',                pr.overtime_pay,
        'custom_allowances_total',     pr.custom_allowances_total,
        'custom_allowances_breakdown', pr.custom_allowances_breakdown,
        'gross_salary',                pr.gross_salary,
        'leave_deduction',             pr.leave_deduction,
        'late_deduction',              pr.late_deduction,
        'labor_ins_employee',          pr.labor_ins_employee,
        'health_ins_employee',         pr.health_ins_employee,
        'legal_deduction_total',       pr.legal_deduction_total,
        'legal_deduction_breakdown',   pr.legal_deduction_breakdown,
        'total_deductions',            pr.total_deductions,
        'net_salary',                  pr.net_salary
      ) ORDER BY pr.pay_period DESC), '[]'::json)
...
GRANT EXECUTE ON FUNCTION public.liff_get_my_payroll_records(text, int) TO anon, authenticated;
