-- 強制 DROP + 重建 v2 函式，PostgREST schema cache 沒抓到
-- (上個 migration NOTIFY 似乎沒生效)

DROP FUNCTION IF EXISTS public.liff_complete_task_v2(text, int);
...
BEGIN
  ...
  -- chain 模式：尚無 task_confirmations → 解析 step 0 並建立
