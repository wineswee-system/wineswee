-- =============================================
-- 薪資計算：支援全薪 / 半薪 / 無薪 假別
-- 原本只認「事假」當扣薪 → 加入半薪假（病假/生理假）
--
-- 依勞基法：
--   全薪假  特休、婚假、喪假、公假、產假、公傷病假、陪產假、補休 → 不扣
--   半薪假  普通病假（前30天）、生理假                            → 扣 50%
--   無薪假  事假、無薪假                                         → 扣 100%
-- =============================================
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Zero attendance bonus if any tardiness or absence (含半薪)
