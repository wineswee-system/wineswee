-- ============================================================
-- 砍 'applicant_supervisor' target_type（用 employees.reporting_to）
-- 原因：這個專案組織圖完整（store/dept/section 都有 manager），不會用到
-- 「個人特殊指派」場景。砍掉讓 UI 從 10 種降到 9 種，admin 不會選錯。
--
-- 既有 row 若是這個 type → 改成 applicant_dept_manager（最通用的 fallback）
-- ============================================================

BEGIN;

-- 1. 砍既有 row 的 applicant_supervisor → applicant_dept_manager
...
ALTER TABLE public.approval_chain_steps
  ALTER COLUMN target_type SET DEFAULT 'applicant_dept_manager';

-- 3. 砍舊 CHECK 重建（不含 applicant_supervisor）
DO $$
DECLARE r record;
BEGIN
  FOR r IN
...
-- 4. 更新 resolver — 拿掉 applicant_supervisor 那一段
...
-- 5. 更新 preview — 拿掉 applicant_supervisor case
