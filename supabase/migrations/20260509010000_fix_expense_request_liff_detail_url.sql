-- ════════════════════════════════════════════════════════════
-- Fix: 經費申請 LINE 卡「看完整詳情 / 查看詳情」改用 /approve/expense-request?id=X
--
-- 之前 LIFF deeplink 是 /approve?type=expense_request&id=X
-- 改為 /approve/expense-request?id=X，讓 LIFF 直接開到對應申請的審核畫面
-- ════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ── footer 按鈕 ──
  -- ★ LIFF deeplink → /approve/expense-request?id=X（直接開到該申請的審核畫面）
  ...
  IF p_event = 'step_assigned' THEN
    ...
    END IF;
  ELSE
    -- approved / rejected：只有 LIFF 詳情按鈕
    IF v_liff_url IS NOT NULL THEN
      v_footer_buttons := jsonb_build_array(
        jsonb_build_object(
          'type','button',
