-- ════════════════════════════════════════════════════════════
-- 恢復 expense_request carousel — 把 20260509010000 砍掉的第二卡補回來
--
-- Bug：20260509010000_fix_expense_request_liff_detail_url 為了改 LIFF URL，
--      CREATE OR REPLACE _push_expense_request_flex，但**從 v1 改的**，
--      整個 carousel 邏輯（v_bubble2 / 費用明細 / 品項列表）全丟。
--
-- 修法：以 20260508120000 v2（carousel 完整）為基底，把 LIFF URL 改成
--      新 pattern /approve/expense-request?id=X（保留 20260509010000 的修法）
--
-- 維持：
--   • 第一卡：申請人 + 金額/項目/科目/門市 + 申請說明 + 📎 附件 + footer 按鈕
--   • 第二卡：費用明細（供應商 + 品項/數量/小記 + 合計）
--   • 有 supplier or items → carousel；否則 single bubble
-- ════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF p_liff_id IS NOT NULL AND p_liff_id <> '' THEN
    -- 新 pattern: /approve/expense-request?id=X (對齊 20260509010000 修法)
