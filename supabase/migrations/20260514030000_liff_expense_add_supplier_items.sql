-- ════════════════════════════════════════════════════════════
-- LIFF 簽核中心：expense_requests / expense_settles 補欄位
-- 2026-05-14
--
-- 反饋：LIFF 看 expense_request 細節時缺以下欄位（Web 有但 LIFF 沒）：
--   - supplier   (供應商)
--   - items      (品項明細 jsonb 陣列：品名/數量/單價/小計)
--   - notes      (備註，僅 expense_requests 段缺；settles 已有)
--
-- 修法：在 liff_list_pending_approvals 的兩個 json_build_object 補欄位。
--       純加欄位、業務邏輯零變動。
-- ════════════════════════════════════════════════════════════
  ...
  SELECT json_build_object(
    ...
    'expense_requests', (
      SELECT COALESCE(json_agg(json_build_object(
        ...
        'current_step_approver', public._chain_step_display_names(cur_step.id, er.employee_id)
...
-- 驗證：func 還在 + 簽名正確
SELECT
  (pg_get_functiondef('public.liff_list_pending_approvals'::regproc) LIKE '%''supplier'', er.supplier%') AS has_supplier,
  (pg_get_functiondef('public.liff_list_pending_approvals'::regproc) LIKE '%''items'', er.items%')       AS has_items,
  (pg_get_functiondef('public.liff_list_pending_approvals'::regproc) LIKE '%''notes'', er.notes%')       AS has_notes;
