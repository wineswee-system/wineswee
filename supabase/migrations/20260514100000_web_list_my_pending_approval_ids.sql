-- ════════════════════════════════════════════════════════════
-- web_list_my_pending_approval_ids: web 版「我能簽的單據 ID 清單」
-- 2026-05-14
--
-- 目的：HR 表單頁的「核准/拒絕」按鈕原本只判 status='待審核' →
-- admin/super_admin 也能按非自己關卡的單。改成前端先呼這支 RPC 拿
-- 「目前我這關該簽的 id 集合」，按鈕加 set.has(id) 判定。
--
-- 邏輯複用 LIFF 那支 liff_list_pending_approvals，差異：
--   1. 用 auth.uid() 解 employee 而非 line_user_id
--   2. 只回 id list 不回完整 row（前端已有資料）
--   3. ★ 自己不能簽自己：加 applicant_id != emp.id filter
-- ════════════════════════════════════════════════════════════
  ...
  SELECT json_build_object(
    ...
    'leave_requests', (
      ...
      WHERE l.organization_id = emp.organization_id
        AND l.status = '待審核'
        AND COALESCE(l.employee_id, -1) <> emp.id  -- 自己不能簽自己
        AND (
