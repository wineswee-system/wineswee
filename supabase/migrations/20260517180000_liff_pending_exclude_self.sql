-- ════════════════════════════════════════════════════════════════════════════
-- liff_list_pending_approvals — LIFF 跟 Web 完整對齊
--
-- 目標：LIFF 看到的「我的待簽」= Web 看到的，無差異。
-- 修法 (4 件)：
--   1. HR 5 表（leave/overtime/trip/correction/expense）改成 chain-aware
--      - 有 chain → 用 _employee_matches_chain_step 精準解當前 step
--      - 沒 chain → 退回到 _resolve_hr_approver_ids (HR 組織圖)
--      → 解決「LIFF 列得到不是我這關的單」
--   2. 加 resignation_requests / leave_of_absence_requests / personnel_transfer_requests
--      → 之前只在 Web 看得到，LIFF 看不到（HR 異動 3 表）
--   3. 加 expense_settles
--      → 核銷之前只在 Web 看得到
--   4. 全部加 self-exclude filter
--      → 解決「自己提的單列在自己的待簽」
--
-- 保留 LIFF 獨有（Web 沒有，但 LIFF 一直有的）：
--   - off_requests (希望休)
--   - shift_swaps_for_peer / shift_swaps_for_manager (換班)
--   - task_confirmations (透過另一支 RPC liff_list_my_task_confirmations 處理)
--
-- 對齊：web_list_my_pending_approval_ids (20260514100000)
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- ─── LIFF 獨有：task / shift_swap / off_request ─────────────────
    'task_confirmations', '[]'::json,  -- 仍由 liff_list_my_task_confirmations 處理
