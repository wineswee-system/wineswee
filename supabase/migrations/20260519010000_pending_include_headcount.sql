-- ════════════════════════════════════════════════════════════════════════════
-- 待簽清單 RPC：加 headcount_requests 區段
-- ────────────────────────────────────────────────────────────────────────────
-- 配合新表 headcount_requests，讓 Dashboard 我的待簽（Web）跟 LIFF 我的待簽
-- 都能看到該關卡 / 加簽人應簽的人力需求單。
--
-- 1:1 重寫前版 (20260518130000)，唯一新增是兩個 RPC 都多 'headcount_requests'
-- 區段；其他段（HR 5 / HR 異動 3 / expense / shift_swaps / off_requests /
-- task_confirmations）完全不動，含 self-exclude / chain match / OR
-- _has_pending_extra_for_me。
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'headcount_requests', (
      ...
      WHERE h.organization_id = emp.organization_id
        AND h.status = '申請中'
        AND COALESCE(h.employee_id, -1) <> emp.id
        AND (
          (h.approval_chain_id IS NOT NULL AND cs.id IS NOT NULL
