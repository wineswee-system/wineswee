-- ════════════════════════════════════════════════════════════════════════════
-- 我已簽核清單 RPC (給簽核中心「已簽核」tab 用)
-- ────────────────────────────────────────────────────────────────────────────
-- 資料源 = UNION
--   (1) approval_step_history (approver_id = me, action IN approved/rejected)
--       → 走 chain 的簽核 (resignation/loa/transfer/leave/overtime/.../expense_request)
--   (2) approval_extra_steps (assignee_id = me, status IN approved/rejected)
--       → 加簽
--
-- request_type 字串 (history) vs source_table (extra) 不一致，內部 normalize
-- 成 table 名再 join 原表拿申請人 / 狀態 / summary。
--
-- 3 個 function:
--   _list_my_signed_approvals(p_emp_id INT, p_year_month TEXT)  共用 helper
--   web_list_my_signed_approvals(p_year_month TEXT)              Web wrapper (auth.uid)
--   liff_list_my_signed_approvals(p_line_user_id, p_year_month)  LIFF wrapper
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  WITH chain_signed AS (
    SELECT
      ...
      ash.action              AS my_action,        -- approved / rejected
  ...
  extra_signed AS (
    SELECT
      -- source_table 是 'leave_requests' 表名；轉成 history 的短名以利統一 mapping
      ...
      es.status               AS my_action,        -- approved / rejected
...
-- ─── Web wrapper：用 auth.uid 找對應 emp ────────────────────────────────────
CREATE OR REPLACE FUNCTION public.web_list_my_signed_approvals(
...
BEGIN
  ...
  END IF;
  SELECT id INTO v_emp_id FROM employees WHERE auth_user_id = v_uid LIMIT 1;
  IF v_emp_id IS NULL THEN
    RETURN json_build_object('error', 'EMPLOYEE_NOT_FOUND');
  END IF;
  RETURN public._list_my_signed_approvals(v_emp_id, p_year_month);
END $$;

GRANT EXECUTE ON FUNCTION public.web_list_my_signed_approvals(TEXT) TO authenticated;


-- ─── LIFF wrapper：用 line_user_id 找對應 emp ──────────────────────────────
CREATE OR REPLACE FUNCTION public.liff_list_my_signed_approvals(
  p_line_user_id TEXT,
  p_year_month   TEXT DEFAULT NULL
) RETURNS json
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public
...
BEGIN
  ...
  IF emp.id IS NULL THEN
    RETURN json_build_object('error', 'EMPLOYEE_NOT_FOUND');
  END IF;
  RETURN public._list_my_signed_approvals(emp.id, p_year_month);
END $$;

GRANT EXECUTE ON FUNCTION public.liff_list_my_signed_approvals(TEXT, TEXT) TO authenticated, anon;
