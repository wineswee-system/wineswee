-- ════════════════════════════════════════════════════════════════════════════
-- liff_list_pending_approvals form_submissions 區段補 template_fields
-- ────────────────────────────────────────────────────────────────────────────
-- LIFF Approve 自訂表單 row 只看到模板名 + 關卡，看不到實際欄位資料跟附件。
-- 因為 220003 寫 liff_list_pending_approvals 時沒把 form_templates.fields 帶回，
-- LIFF 不知道有哪些欄位。
--
-- 修法：只動 form_submissions 區段，加一個 template_fields 進回傳 jsonb。
-- 其他區段（leave/overtime/expense_request 等）完全不動，避免漏改。
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- ★ form_submissions：補 template_fields + attachments resolve（2026-05-20）
    'form_submissions', (
      SELECT COALESCE(json_agg(json_build_object(
        'id', s.id, 'template_id', s.template_id, 'template_name', t.name,
        'template_fields', t.fields,  -- ★ 新增：欄位定義 (jsonb)，LIFF 用來知道怎麼 render
