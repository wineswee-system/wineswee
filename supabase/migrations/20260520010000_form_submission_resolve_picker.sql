-- ════════════════════════════════════════════════════════════════════════════
-- form_submissions：picker 欄位 ID 自動 resolve 成 name（給 LIFF 顯示用）
-- ────────────────────────────────────────────────────────────────────────────
-- 慘案：LIFF Approve 自訂表單 row picker 欄位顯示 "#148" 不是 "黃蘊珊"，因為
-- data jsonb 存的是 ID，LIFF 沒辦法走 RLS 直查 employees/departments/stores 把
-- ID 解成名字。
--
-- 修法：
--   1. helper _resolve_form_submission_data(data, fields) 走 fields jsonb，
--      對 employee/department/store_picker 把 data[key] 的 ID 換成 name
--   2. liff_list_pending_approvals form_submissions 區段同時回傳 data_resolved
--      （解析過的）+ data（原始）兩份；LIFF prefer data_resolved
-- ════════════════════════════════════════════════════════════════════════════

BEGIN;

-- ─── 1. resolver helper：吃 data + fields，吐回填名字後的 jsonb ────────
CREATE OR REPLACE FUNCTION public._resolve_form_submission_data(
  p_data   jsonb,
  p_fields jsonb
) RETURNS jsonb
...
DECLARE
  ...
  v_name   text;
BEGIN
  IF jsonb_typeof(p_fields) <> 'array' THEN RETURN v_result; END IF;
  ...
  FOR v_field IN SELECT * FROM jsonb_array_elements(p_fields) LOOP
    ...
    -- 只處理 picker 類型
    CONTINUE WHEN v_type NOT IN ('employee_picker', 'department_picker', 'store_picker');
    CONTINUE WHEN v_key IS NULL OR v_result->v_key IS NULL;

    -- jsonb 數字/字串都接：先試 ::int，失敗就 skip
    BEGIN
      v_id := (v_result->>v_key)::int;
    ...
    ELSIF v_type = 'department_picker' THEN
      SELECT name INTO v_name FROM departments WHERE id = v_id;
    ELSIF v_type = 'store_picker' THEN
      SELECT name INTO v_name FROM stores WHERE id = v_id;
    END IF;

    IF v_name IS NOT NULL THEN
      v_result := jsonb_set(v_result, ARRAY[v_key], to_jsonb(v_name));
    END IF;
  END LOOP;
...
GRANT EXECUTE ON FUNCTION public._resolve_form_submission_data(jsonb, jsonb) TO authenticated, anon;


-- ─── 2. 重寫 liff_list_pending_approvals 只改 form_submissions 區段加 data_resolved ──
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- ★ form_submissions：data 內 picker ID 解成 name (2026-05-20)
    'form_submissions', (
      SELECT COALESCE(json_agg(json_build_object(
        ...
        'data',          s.data,                                                  -- 原始（含 picker ID）
        'data_resolved', public._resolve_form_submission_data(s.data, t.fields),  -- ★ picker ID 已換成 name
