-- ════════════════════════════════════════════════════════════════════════════
-- liff_approve_request：補 form_submission 分支
-- ────────────────────────────────────────────────────────────────────────────
-- 問題：liff_approve_request 支援 leave/overtime/trip/correction/expense/
--       expense_request 共 6 種，但自訂表單 (form_submissions) 完全沒有 handler。
-- 症狀：簽核人在 LINE 收到通知、按「核准」→ LIFF 呼叫 liff_approve_request
--       with p_type='form_submission' → 落到 INVALID_TYPE → 什麼都不做 →
--       chain 不推進 → 下一關 / 結案通知都不發。
-- 修法：在 INVALID_TYPE 前加 form_submission 分支，委派給已有完整 chain-advance
--       + LINE trigger 邏輯的 form_submission_chain_approve RPC。
-- 注意：完整 CREATE OR REPLACE 以免 partial rewrite 災難（見 feedback_migration_partial_overwrite_disaster）
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ════ form_submission 走 form_submission_chain_approve ════
  -- 委派給已有完整 chain-advance + LINE trigger 邏輯的 RPC，避免重複實作。
  -- form_submission_chain_approve 內部驗 _employee_matches_chain_step 和 status，
  -- 所以這裡只需解 emp 後直接呼叫。
