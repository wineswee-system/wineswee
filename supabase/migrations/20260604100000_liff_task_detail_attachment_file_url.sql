-- ════════════════════════════════════════════════════════════════════════════
-- liff_get_task_detail.attachments 加 file_url 欄
--
-- 背景：主系統上傳的附件存 file_url（公開 URL），storage_path 為 NULL；
--      LIFF 上傳的存 storage_path，file_url 為 NULL。
--      之前 RPC 只回 storage_path → 主系統來的附件 LIFF 點下載拿到 null
--      → getPublicUrl(null) 噴 'Cannot read properties of null'。
--
-- 兩個欄都回，前端 viewFile 兩個都認（已修）。
-- 其餘邏輯不動 — 完整 CREATE OR REPLACE 避免 partial overwrite。
-- ════════════════════════════════════════════════════════════════════════════
...
DO $$ BEGIN
  RAISE NOTICE 'liff_get_task_detail.attachments 已加 file_url 欄';
END $$;
