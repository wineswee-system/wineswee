-- ════════════════════════════════════════════════════════════════════════════
-- RESTORE liff_approve_request — 復活 expense_settle 分支
--
-- 慘案再現：使用者 LIFF 按「核准核銷」噴「單據類型錯誤」(INVALID_TYPE)。
-- Studio 查 pg_get_functiondef(liff_approve_request) → 不含 'expense_settle' 字串。
-- 結論：老闆 Studio hotfix 又 partial overwrite 洗掉了 expense_settle 分支
-- （與 2026-05-12 記憶 [[feedback-migration-partial-overwrite-disaster]] 同款）。
--
-- 修復做法：完整重貼 20260602110000_settle_notify_snapshot_fix.sql 內的
-- canonical liff_approve_request 全文 — 包含：
--   - HR A 類 (leave/overtime/trip/correction/expense) 含 snapshot
--   - expense_request 走 approval_chain
--   - expense_settle  走 settle_chain + snapshot + extra step guard + 自動產分錄
--
-- 不動任何其他物件。純 CREATE OR REPLACE + GRANT。
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  SELECT pg_get_functiondef(oid) ILIKE '%expense_settle%' INTO v_has
    FROM pg_proc WHERE proname='liff_approve_request' AND pronamespace='public'::regnamespace LIMIT 1;
  IF v_has THEN
    RAISE NOTICE 'liff_approve_request 已復活，含 expense_settle 分支';
  ELSE
    RAISE WARNING 'liff_approve_request 還是沒 expense_settle — 請確認沒被另一支 migration 覆蓋';
  END IF;
END $$;
