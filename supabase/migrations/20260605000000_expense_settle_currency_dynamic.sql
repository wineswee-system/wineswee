-- ════════════════════════════════════════════════════════════════════════════
-- 修 _push_expense_settle_flex：核銷階段 LINE flex 卡幣別動態化
--
-- 問題：核銷階段 LINE 通知三處金額固定 'NT$'，
--       但 expense_requests 有 currency 欄位，USD/JPY/EUR/CNY 申請會誤顯示 NT$
--
-- 對齊：20260523160000_expense_currency_dynamic.sql 對「申請階段」(_push_expense_request_flex)
--       的修法 — 解析 v_currency_sym + v_currency_fmt
--
-- 影響三處（同一函式內）：
--   v_amount_str (實際金額)  — 加 sym + currency_fmt
--   v_est_str    (申請金額)  — 加 sym + currency_fmt
--   v_diff_str   (差額)      — 只改 currency_fmt（不加 sym，保持原視覺：+/-1,234）
--
-- 整支 CREATE OR REPLACE 完整貼回，避免 partial rewrite 洗掉邏輯
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  v_currency_fmt := CASE COALESCE(v_req.currency, 'TWD')
    WHEN 'JPY' THEN 'FM999,999,999,999'
    WHEN 'TWD' THEN 'FM999,999,999,999'
    ELSE 'FM999,999,999,990.00'
  END;
