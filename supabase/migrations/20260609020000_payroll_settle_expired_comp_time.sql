-- ════════════════════════════════════════════════════════════════════════════
-- generate_payroll — 在 20260608080000 之上再加 1 項
--
-- 1—4. 沿用 20260608080000 的四項修正（曆日 prorate / join_date 過濾 / v_net CEIL）
--
-- 5. 過期補休自動兌現（NEW）
--    員工 OT 申請選 comp_time → comp_time_ledger 記錄；過期未用月結時兌現為加班費。
--    呼叫 helper _settle_expired_comp_time(emp_id, run_id, month_end) 返回該員工
--    所有過期未結 ledger 加總金額，加進 v_ot_pay。
--
-- 對 20260608080000 的精準 patch：2 處（DECLARE 加變數 + ot_pay 計算後加呼叫）。
-- ════════════════════════════════════════════════════════════════════════════
