-- ════════════════════════════════════════════════════════════════════════════
-- generate_payroll — 進位策略統一 + 小時級請假修正（員工有利原則）
--
-- 1—6. 沿用 20260609060000 所有修正（包含進位策略）
--
-- 7. 小時級請假扣款修正（NEW）：
--    之前 PG 算請假扣款只用 LEAST(end_date)-GREATEST(start_date)+1 算天數，
--    完全忽略 hours 欄位 → 員工請 4 小時事假被當整天扣（多扣半天薪）。
--    現在依 unit 欄位分流：
--      - unit='hour' → 用 hours × hourly_rate
--      - unit='day' / NULL → 用 days × daily_rate（原邏輯）
--    跟 JS payrollCalc.js 對齊。
-- ════════════════════════════════════════════════════════════════════════════
