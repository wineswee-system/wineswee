-- ════════════════════════════════════════════════════════════════════════════
-- generate_payroll — 勞健保 toggle 化 + 動態級距
--
-- 1—7. 沿用 20260609070000 所有修正
--
-- 8. 勞健保保不保改成 toggle（NEW）：
--    員工資料表有 labor_insurance / health_insurance BOOLEAN 但之前完全沒用 →
--    無論 toggle on/off，只要設了 grade 就扣。
--    現在改成：
--      - labor_insurance = false → 不扣勞保（員工自付 + 雇主負擔都歸 0）
--      - health_insurance = false → 不扣健保
--      - toggle on 但 grade NULL → 依當月投保薪資自動找最接近級距
--      - toggle on 且 grade 已設 → 用該級距（手動覆寫）
--    投保薪資基準：v_base + 主管加給 + 餐補 + 交通 + 全勤 + custom_total
--    （跟 JS baseForInsure 對齊）
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- 事假/無薪假：unit='hour' 走 hours、其他走 date diff
      SELECT
        COALESCE(SUM(CASE
      ...
      -- 病假/生理假：unit='hour' 走 hours、其他走 date diff
      SELECT
        COALESCE(SUM(CASE
          WHEN COALESCE(unit, 'day') = 'hour' THEN 0
          ELSE LEAST(end_date, v_month_end)::date - GREATEST(start_date, v_month_start)::date + 1
      ...
      -- 含小時 → 換算成天（顯示用，存到 payroll_records.leave_days_deducted）
