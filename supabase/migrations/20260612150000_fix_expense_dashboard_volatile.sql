-- Fix: remove STABLE — 某些 PostgREST 版本對 STABLE function 的 POST 請求處理有問題
    ...
    'expense_rows', (
      ...
      FROM (
        ...
        WHERE organization_id = v_org_id AND deleted_at IS NULL
          ...
          AND (p_account_codes IS NULL OR array_length(p_account_codes,1) IS NULL OR account_code = ANY(p_account_codes))
    ...
    'non_exp_rows', (
      ...
      FROM (
        ...
        WHERE ft.scope = 'business_non_expense' AND fs.deleted_at IS NULL
          ...
          AND (p_template_ids IS NULL OR array_length(p_template_ids,1) IS NULL OR fs.template_id = ANY(p_template_ids))
