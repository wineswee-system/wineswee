-- 計件件數按月存：新表 employee_piece_counts + 計薪改讀當月件數（找不到才用舊 current_piece_count）
-- 2026-06-17
-- 函式本體 100% 複製自 20260615180000（快照確認 live 無 drift），只改件數來源那一行。

BEGIN;

-- ── 1. 件數按月表 ──
CREATE TABLE IF NOT EXISTS public.employee_piece_counts (
...
ALTER TABLE public.employee_piece_counts ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS epc_org_all ON public.employee_piece_counts;
CREATE POLICY epc_org_all ON public.employee_piece_counts
  FOR ALL TO authenticated
  USING (organization_id = current_employee_org())
  WITH CHECK (organization_id = current_employee_org());

GRANT SELECT, INSERT, UPDATE, DELETE ON public.employee_piece_counts TO authenticated;
GRANT ALL ON public.employee_piece_counts TO service_role;
