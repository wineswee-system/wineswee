-- ════════════════════════════════════════════════════════════════════════════
-- 計薪掛鉤：有「提早下班登記」(early_leave_records)的那天 → 跳過早退扣款
-- 2026-06-18  Phase 2（搭配 20260618110000 的 early_leave_records 表 + 表單）
--
-- 店方安排提早下班 → 店長記一筆 → 那天不算早退扣款（底薪本來就照實際打卡時數算，
-- 直接算對，班表不用動）。只跳「早退」、不動「遲到」（遲到是員工自己的事）。
--
-- ★ 本檔以 live 版 _compute_payroll_for_employee 為基準（db:drift 快照），
--   只用機械式字串替換加 3 處：
--     1) 主計算 early SUM 的 FILTER 加 AND NOT has_el
--     2) 兩個子查詢各加 has_el = EXISTS(early_leave_records 當天)
--     3) _early_rows 明細 em 條件加 AND NOT y.has_el
--   已 diff 驗證「只動這幾處、遲到不受影響」。
