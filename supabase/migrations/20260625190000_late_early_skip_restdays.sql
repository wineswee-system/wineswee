-- ════════════════════════════════════════════════════════════════════════════
-- 遲到/早退：國定假日 或 行政遇週末 那天一律不算（沒正常班 → 不該扣遲到早退）
-- 2026-06-25
--
-- 問題：行政人員沒班表 → 遲到/早退退回固定 09:00–18:00。但國定假日/週末(行政的休息日)
-- 進來打卡做加班時，被拿去比固定 9–6 → 誤判遲到+早退（例：林巧玉 4/03國定、4/18週六）。
-- 修法：主計算 + _late_rows + _early_rows 三個子查詢，各加排除條件：
--   該日是國定假日(holidays.is_workday=false) 或 (行政 admin 且 DOW∈{0,6}週末) → 不納入。
-- 門市/PT 四週變形的排休日本來就因「無班表 actual 時間 → ast/ae=NULL」被排除，不受影響。
--
-- ★ 以 live 版 _compute_payroll_for_employee（20260618120000）為基準，
--   只加 holidays join + WHERE 排除條件，其餘逐字不動；diff 驗證僅這幾處。
