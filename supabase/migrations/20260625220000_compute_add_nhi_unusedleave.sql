-- ════════════════════════════════════════════════════════════════════════════
-- B2.1：試算 _compute 補「離職特休結清 + 二代健保補充保費」(讓試算=入帳)
-- 2026-06-25
--
-- 為了 B2「入帳改呼叫試算函式」,試算要先補入帳有、試算沒有的兩塊:
--   1. 離職特休結清(unused_leave_payout)：離職當月把沒休完特休折現,計入 gross
--   2. 二代健保補充保費(nhi_supplementary)：加班費超投保 + 高額獎金累計>4倍投保 → ×2.11%
--      門檻用「覈實投保 v_health_insured」(對齊試算保險邏輯,非存的grade)
--      讀 annual_bonus_tracker(唯讀,不寫;寫由入帳 generate_payroll 負責)
--
-- ★ 以 live 版 _compute_payroll_for_employee（20260625190000）為基準,只加上述兩塊
--   + 對應 return 欄位,其餘逐字不動;diff 驗證僅這幾處。idempotent：CREATE OR REPLACE。
-- ════════════════════════════════════════════════════════════════════════════
...
GRANT EXECUTE ON FUNCTION public._compute_payroll_for_employee(integer, text) TO authenticated, service_role;
