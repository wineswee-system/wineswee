...
-- 1. 現有 store_audit_items 記錄重編（一次 CASE 避免連鎖覆寫）
UPDATE public.store_audit_items
   SET category_code = CASE category_code
     ...
     ELSE category_code
   END
 WHERE category_code IN ('四', '五', '六');

-- 2. 更新 _create_store_audit_default_items（新建稽核單用新編號）
...
BEGIN
  ...
  (p_audit_id, '三', '服務層面', 8, '客人結帳離開，應立即完成收桌、消毒桌面及檢查地面髒污', 3),
  -- 四、暢飲規範（原五）
  (p_audit_id, '四', '暢飲規範', 1, '暢飲酒款依照規定品項及數量放置出來', 4),
  (p_audit_id, '四', '暢飲規範', 2, '暢飲規則及取酒機使用方式，確實告知客人', 4),
  (p_audit_id, '四', '暢飲規範', 3, '遵照暢飲規定執行：同行皆須參與活動，每人/1hr/$290', 4),
  (p_audit_id, '四', '暢飲規範', 4, '暢飲時間計算：給杯子才開始計算時間、收回杯子才可停時間', 4),
  (p_audit_id, '四', '暢飲規範', 5, '暢飲空瓶檢核：皆須依照【標籤流水號】開瓶', 4),
  (p_audit_id, '四', '暢飲規範', 6, '客人若要更換酒款飲用，須提供「洗杯」服務', 4),
  (p_audit_id, '四', '暢飲規範', 7, '打烊前，單杯酒及暢飲酒開瓶，須標示開瓶當天日期', 4),
  -- 五、其他（原六）
  (p_audit_id, '五', '其他', 1, '『零用金』數額正確', 4),
  (p_audit_id, '五', '其他', 2, '展示行銷活動文宣、活動到期日於當天打烊下架文宣', 4),
  (p_audit_id, '五', '其他', 3, '新商品入倉確實檢查，核對驗收及即時上架銷售', 4),
  (p_audit_id, '五', '其他', 4, '確實按照帳單結帳，不可隨意折扣', 4),
  (p_audit_id, '五', '其他', 5, '「每日店務交接表」、「每日工作檢核表」確實填寫並簽名確認', 4),
  (p_audit_id, '五', '其他', 6, '店內庫存【抽查3樣】如有錯誤，需與當班人員確認「現場實際數量」再記錄簽名', 4);
