-- ════════════════════════════════════════════════════════════════════════════
-- liff_list_pending：全表 pending 判斷改讀快照（推廣 expense_request 的 pattern）
-- 2026-07-02
--
-- 承 20260702820000（費用申請已驗證）。把同一 pattern 套到其餘有快照的 5 段：
--   leave_request / overtime_request / correction / expense_settle / form_submission
--   → 有快照優先讀 _employee_matches_snapshot_step(申請當下定住，改 chain 不影響)、
--     沒快照(舊單) fallback _employee_matches_chain_step(live)、HR fallback 原樣保留。
--
-- trip/resignation/loa/transfer/headcount/expenses 目前無快照(0 筆) → 維持 live 未動
--   （要根治這幾種得先讓它們生快照，另議）。
--
-- 定義取自 live DB(_dump_function_defs)只局部改 6 段判斷，避免 drift + 巨函式 partial rewrite。
-- ════════════════════════════════════════════════════════════════════════════
