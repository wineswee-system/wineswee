-- 修：補打卡核准後工時不自動算（total_hours 空）
-- 2026-07-06
-- 根因：_apply_correction_to_attendance 把補登的 clock_in/clock_out 寫進 attendance_records，
--   但沒重算 total_hours → 工時顓示 '-'。
-- 修：在函式尾端加一段重算 total_hours（兩端都有才算、扣休息 calc_shift_rest_minutes、跨午夜+24h）。
-- 手法：dump live 只加尾段，其餘不動。idempotent(CREATE OR REPLACE)。
