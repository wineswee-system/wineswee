-- 勞退接上開關 + 全部人關掉
-- 2026-07-08
-- 現況：計薪三支(preview→_compute / generate)算雇主勞退 6% 是「無條件」，
--   完全沒讀 employees.pension 這個 UI 開關 → 關 toggle 對薪水沒效果。
-- 本 migration：
--   ① 讓 _compute(涵蓋 preview) 與 generate_payroll 的雇主勞退改吃 pension 開關：
--      pension=true → 級距 × pension_rate%(預設6)；pension=false → 0
--   ② 全部員工 pension=false（只有一間 org，不加 org 條件）
-- 手法：抓 live pg_get_functiondef 完整定義，只替換勞退那幾行(assert 各 1 處)，
--   其餘一字不動 → 不會洗掉別的計薪邏輯。idempotent。
...
BEGIN
  ...
  FOR rec IN
    SELECT
      ...
      COALESCE(e.pension, false)                  AS pension_enrolled,
      COALESCE(e.pension_rate, 6)                 AS pension_rate,
  ...
  LOOP
    ...
    BEGIN
      ...
      v_pension_er  := CASE WHEN rec.pension_enrolled THEN CEIL(LEAST(v_base, 150000) * (rec.pension_rate/100.0)) ELSE 0 END;
...
-- ② 全部員工關掉勞退（雇主提繳）
UPDATE public.employees SET pension = false WHERE pension IS DISTINCT FROM false;
