-- 計薪 #1：班表為準（preview/_compute）
-- 2026-07-08
-- 需求：PT/時薪底薪改吃「班表排定工時」而非打卡；取消打卡遲到/早退/曠職扣款；
--   打卡只留記錄。休息規則(看班長度)：<5h→0、5~9h→30分、>=9h→60分。
-- 兩個 helper：
--   _shift_seg_hours(s,e)：單一時段 paid hours（跨午夜+24h、扣休息）
--   _scheduled_work_hours(emp,period)：整月加總（含分段班 shift_2、跳請假日 leave_request_id）
-- 只改 _compute(preview)；generate_payroll(入帳) 待 preview 比對確認後另辦。idempotent。

CREATE OR REPLACE FUNCTION public._shift_seg_hours(s time, e time)
RETURNS numeric LANGUAGE sql IMMUTABLE AS $$
  ...
  SELECT COALESCE(
    GREATEST(0, m - CASE WHEN m >= 540 THEN 60 WHEN m >= 300 THEN 30 ELSE 0 END) / 60.0,
    0)
  FROM x
$$;

CREATE OR REPLACE FUNCTION public._scheduled_work_hours(p_emp_id int, p_period text)
RETURNS numeric LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT COALESCE(SUM(
      public._shift_seg_hours(sc.actual_start,   sc.actual_end)
    + public._shift_seg_hours(sc.actual_start_2, sc.actual_end_2)
  ), 0)
...
GRANT EXECUTE ON FUNCTION public._shift_seg_hours(time,time) TO authenticated, anon, service_role;
GRANT EXECUTE ON FUNCTION public._scheduled_work_hours(int,text) TO authenticated, anon, service_role;
...
BEGIN
  ...
  ELSIF v_is_hourly THEN
    v_base_salary := ceil(COALESCE(v_ss.hourly_rate,0) * v_sched_hours);
  ELSE
  ...
  -- ── 扣款金額 ──
  v_late_deduction  := 0;  -- 班表為準：取消打卡遲到扣款
  v_early_deduction := 0;  -- 班表為準：取消打卡早退扣款
  ...
  v_absence_deduct := 0;  -- 有排班就給薪：不自動扣曠職
