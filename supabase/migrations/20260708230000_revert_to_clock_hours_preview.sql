-- 計薪 #1 回滾：回到打卡制（preview/_compute）
-- 2026-07-08  老闆決策：班表制不用了，回原本打卡制(A)。
-- 改回：PT/時薪底薪 = 打卡時數 v_hours × 時薪；遲到/早退/曠職扣款回原公式。
-- 保留：#2 固定投保級距(v_labor_ins_base)、雇主勞退6% gate、scheduledHours 顯示欄(僅參考不影響計薪)。
-- helper _shift_seg_hours/_scheduled_work_hours 保留(供 scheduledHours 顯示)。
-- 前提：打卡資料要乾淨(6月出勤覆蓋待做)。dump live 只換 4 行(各 assert 1)，其餘不動。idempotent。
