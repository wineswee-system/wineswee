-- 加班日別改讀班表（preview/_compute）
-- 2026-07-08  bug：加班費率日別(平日/休息日/例假/國定)沒讀員工班表——
--   信 overtime_requests.ot_category(104匯入全寫死 weekday)，空的才用「日曆星期」fallback，
--   完全無視班表的例假/休息/國定 → 例假出勤被當平日三桶(少發加班費)。
-- 修：新 helper _ot_category 讀該員該日 schedules.shift + holidays——
--   班表含「例假」→weekly_off、「休息」→restday、「國定」或 holidays 有→holiday；
--   其他日維持原邏輯(ot_category 或日曆星期 fallback)。
-- _compute 5 處分類(3 算錢+2 顯示標籤)全改呼叫 helper。
-- 僅 preview；generate_payroll(入帳) 待確認後另辦。dump live 只換這 5 段。idempotent。

CREATE OR REPLACE FUNCTION public._ot_category(p_emp_id int, p_date date, p_ot_category text)
RETURNS text LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH sc AS (
    ...
    SELECT EXISTS(SELECT 1 FROM public.holidays WHERE date = p_date) AS is_hol
  )
  SELECT CASE
    WHEN sc.shifts LIKE '%例假%' THEN 'weekly_off'
    WHEN sc.shifts LIKE '%休息%' THEN 'restday'
    WHEN sc.shifts LIKE '%國定%' OR hol.is_hol THEN 'holiday'
    ELSE COALESCE(NULLIF(p_ot_category,''),
      CASE extract(dow from p_date)::int WHEN 0 THEN 'weekly_off' WHEN 6 THEN 'restday' ELSE 'weekday' END)
  END
  FROM sc, hol
$$;
GRANT EXECUTE ON FUNCTION public._ot_category(int,date,text) TO authenticated, anon, service_role;
  ...
  RETURN jsonb_build_object(
    ...
    '_ot_rows', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('date', request_date, 'hours', ot_hours,
        'category', public._ot_category(p_emp_id, request_date, ot_category)) ORDER BY request_date)
