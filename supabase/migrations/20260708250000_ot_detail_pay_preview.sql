-- 加班明細帶逐筆金額 _pay（preview/_compute）
-- 2026-07-08  修顯示：加班明細每筆加 _pay（用 _ot_pay_zh 依分類+時薪制逐筆算），
--   讓前端顯示正確金額/倍率(不再 fallback 平日三桶)。搭配前端補「例假」標籤。
-- 只加 detail JSON 的 _pay 欄，不動計薪聚合。idempotent。
