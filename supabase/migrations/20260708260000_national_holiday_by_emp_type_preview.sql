-- ③ 國定假日改依員工類型判定（preview/_compute）
-- 2026-07-08  正職門市的國定假不看日曆，看班表(國定假排哪天)；兼職/行政看日曆。
--   例：陳嘉益(門市)6/19端午正常上大夜班、他的國定假排別天 → 6/19 對他=平日，
--   不該有國定出勤加給/國定加班。
-- 判斷法(老闆確認)：時薪(兼職) 或 employment_category='admin'(行政) → 看日曆；
--   其餘月薪(正職門市,含 employment_category NULL) → 看班表國定假。
-- 影響：_ot_category(加班日別) + _compute holidayHours(國定出勤加給)。
-- employment_category 在 salary_structures。idempotent。
  ...
  RETURN jsonb_build_object(
    ...
    '_ot_rows', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('date', request_date, 'hours', ot_hours,
        'category', public._ot_category(p_emp_id, request_date, ot_category),
        '_pay', public._ot_pay_zh(ot_hours, v_hourly_rate, public._ot_category(p_emp_id, request_date, ot_category), v_is_hourly)) ORDER BY request_date)
