-- ④ 額外加班費率疊加當天 + 例假/四類 per-day + expose 額外分類 (preview)
-- 2026-07-08  額外加班費率要接續當天加班階梯：
--   額外 = pay(當天加班+額外) − pay(當天只加班)。四類(含例假)全 per-day。
--   (原本額外自己從 ×1.34 重排、例假用月總時數 → 都不對)
-- 順帶 expose _ot_exc_* 分類欄給前端「額外加班費」統計對帳(①)。
-- 僅 preview；入帳 generate_payroll(_compute_ot_pay) 另辦。dump live 只換 2 段。idempotent。
