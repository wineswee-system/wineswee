-- 加班費合併修正 (preview) — 修 migration 互相覆寫 + Bug A/C
-- 2026-07-08  根因：280000(④) dump live 時 260000 未跑 → 蓋掉 260000 的
--   holidayHours=_is_national_holiday 改動。本 migration 從現 live 重補齊，之後不再互蓋。
--   A) holidayHours 換 _is_national_holiday(修門市6/19誤算國定加給)
--   B) _ot_category ELSE 不再信單子存的 ot_category(104匯入寫死holiday)；
--      正職門市沒標特殊=平日、兼職/行政=日曆DOW
--   C) 加班/額外明細改 per-day 分組：額外顯示疊加(marginal)+正確倍率label
-- 新 helper _ot_rate_label(算某時段落在哪些倍率桶)。idempotent。
...
BEGIN
  ...
  IF p_is_hourly AND p_cat IN ('restday','weekly_off','holiday') THEN RETURN '×2.0'; END IF;
...
END $fn$;
GRANT EXECUTE ON FUNCTION public._ot_rate_label(numeric,numeric,text,boolean) TO authenticated, anon, service_role;
