-- 勞退整合固定提繳級距 (preview/_compute)
-- 2026-07-09  勞退提繳工資改讀 employees.labor_pension_grade(已匯入)，跟勞健保固定級距一致；
--   空的才 fallback LEAST(底薪,150000)。雇主6%+自提都吃這個。generate delegate 後入帳同步。
-- dump live 只換 v_wage_grade 一行。idempotent。
