-- 入帳整併:generate_payroll 計算改 delegate 給 _compute_payroll_for_employee(=preview 引擎) — 2026-07-09
-- 背景:昨天的薪資修正(投保固定級距/工時來源/加班 consolidate)只進了引擎(preview),
--   generate_payroll(入帳)自己算一套沒吃到 → 試算≠入帳。整併成單一引擎:
--   ● 算數字(底薪/津貼/加班/投保/所得稅/請假扣款/二代健保/gross/net)全部讀引擎回的 jsonb
--   ● 副作用保留在 generate(引擎唯讀做不到):payroll_runs 建 run、annual_bonus_tracker 累計、
--     legal_deductions 餘額感知扣帳、nhi_supplementary_records 明細
--   ● 法扣:保留 generate 餘額感知邏輯;net/total = 引擎值 − 引擎法扣 + generate 法扣
--     (法扣相同時 net 完全等於 preview → 逐欄對齊)
-- 切換前務必跑 diff 驗證腳本(scripts/_diff_preview_generate.mjs)確認 preview=入帳 再用於正式發薪。
