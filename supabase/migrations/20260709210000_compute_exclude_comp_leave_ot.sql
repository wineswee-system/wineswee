-- _compute 現金加班費排除「折換補休」的加班單(is_comp_leave)
-- 2026-07-09  搭配 20260709200000:選補休的加班不發現金。四個讀 overtime_requests 的區塊
--   (現金四桶/額外疊加/_ot_rows明細/例外明細)都加 AND NOT COALESCE(is_comp_leave,false)。
--   不動:leave_requests(請假)、comp_time_ledger(過期補休兌現另一套)。
--   dump live 定義+context-aware 注入,CREATE OR REPLACE 全覆蓋。idempotent。
