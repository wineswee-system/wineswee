-- 薪資為0時員工自付勞健保歸0(實領不成負),雇主負擔照舊
-- 2026-07-09  洪友銘(PT整月未出勤)應發0卻扣勞保73+健保458→實領-531→撞 chk_salary_positive_net。
--   規則:v_gross<=0 → 員工自付勞健保/自提=0、net=0;雇主負擔不動(公司該付仍付)。
--   dump live + 保險/prorate 全算完後套用。idempotent。generate_payroll/preview 同源生效。
