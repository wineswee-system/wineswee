-- 打卡缺一隻(忘上班/下班卡且沒補卡申請)→ 計薪時讀班表算工時
-- 2026-07-13  情境:PT常一天只打一次卡,那天有上班但打卡不完整→total_hours算不出→白做。
--   規則(整天用班表甲案):有排班(actual_start/end)+缺一隻打卡+非請假+未補卡(補了就完整)
--     → 工時用 _shift_seg_hours(班表);該日不算遲到早退。零打卡不處理(交HR)。
--   即時算(A案):不改 attendance_records,只在 _compute 讀到缺卡時用班表。preview/入帳同源。
