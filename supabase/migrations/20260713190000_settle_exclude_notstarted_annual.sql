-- 離職特休結清排除「未生效」特休（對齊畫面「只看當下能休」）
-- 2026-07-13  _compute B2 特休結清原本加總所有 annual leave_balances，沒排除期間未到的
--   → 畫面顯示 0（未生效）但結算按全額折現，會多發。加 period_start<=v_mend（當月底）才算折現。
--   過期特休保留（§38-4 未休折現本就該發，畫面隱藏≠不折現）。generate_payroll delegate 同源生效。idempotent。
