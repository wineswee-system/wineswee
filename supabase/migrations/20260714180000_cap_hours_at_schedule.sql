-- 工時封頂在班表:本薪以班表為準,超時要走加班申請(另計);打卡只抓遲到早退 — 2026-07-14
-- 決策(用戶 2026-07-14):班表=核准工時,打卡可能亂打;打卡>班表沒申請加班的那段不付。
-- 交叉驗證6月:打卡>班表 278天,其中141天有核准加班(封頂避免本薪+加班重複付),137天沒申請(未核准不付)。
-- 改:兩隻打卡都有 → eff_hours = LEAST(打卡, 班表_shift_seg_hours);無班表(如未排班)→維持打卡不封頂(HR補班表)。
-- 打卡<班表(遲到早退)→ LEAST 取打卡,照扣不受影響。連帶修正 v_holiday_hours(國定同源)。加班另由 overtime_requests(核准)計。
