-- 離職特休折現邏輯修正(A:年資現算天數 + 平均工資日薪) — 2026-07-14
-- 原本:讀 leave_balances 特休餘額(存錯期間就折錯) + 日薪只用本薪。
-- 改為(月薪制):
--   天數 = 年資(join→resign)現算法定特休 - 已休(不靠快照期間,以後離職自動對)
--   日薪 = 平均工資(本薪 + 固定津貼:伙食/主管/夜間/跨店/交通)/30  (§特休折現用平均工資)
-- 兼職/時薪維持原邏輯(時薪×8)。generate_payroll delegate 同源生效。idempotent。
