-- 天災照給薪:時薪員工遇天災(paid)補班表時數 — 2026-07-14
-- disaster_settle_no_shows 對 paid 不建假單(月薪不扣OK),但時薪沒打卡=0工時=拿不到那天的錢,計薪又沒讀天災。
-- 修:時薪員工「當天有排上班班別+沒打卡+該日在天災(paid)宣告涵蓋其門市」→補班表時數進 v_hours。月薪不動。
...
BEGIN
      ...
      CASE
        WHEN ar.clock_in IS NOT NULL AND ar.clock_out IS NOT NULL THEN
          ...
          CASE WHEN s.actual_start IS NOT NULL AND s.actual_end IS NOT NULL
               THEN LEAST(COALESCE(ar.total_hours,0), public._shift_seg_hours(s.actual_start, s.actual_end))
  ...
  IF v_is_hourly THEN
    SELECT COALESCE(SUM(public._shift_seg_hours(s.actual_start, s.actual_end)), 0)
