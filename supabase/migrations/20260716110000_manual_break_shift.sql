-- ════════════════════════════════════════════════════════════════════════════
-- 班別可自訂休息時間(兩頭班) — 2026-07-16
-- 需求:11-14 + 17-22 這種兩頭班,班別記 11-22、休息 3hr → 淨工時 8h。
--   現況全系統「不讀 break_minutes,一律公式」(<5h不扣/5-9h扣30/≥9h扣60)。
-- 做法(預設完全不變,只有勾「自訂休息」才生效):
--   1. shift_definitions 加 manual_break 旗標(break_minutes 欄已存在)
--   2. schedules 加 rest_minutes(null=公式、有值=手動)。派班時把手動休息烤進 schedule
--   3. _shift_seg_hours 新增 3 參數版(帶休息分鐘;null→原公式);舊 2 參數版保留
--   4. _scheduled_work_hours + _compute_payroll_for_employee 改傳 schedules.rest_minutes
-- idempotent。
-- ════════════════════════════════════════════════════════════════════════════

-- 1. 旗標 + schedule 休息欄
ALTER TABLE public.shift_definitions ADD COLUMN IF NOT EXISTS manual_break boolean NOT NULL DEFAULT false;
ALTER TABLE public.schedules         ADD COLUMN IF NOT EXISTS rest_minutes integer;  -- null=用公式;有值=手動休息(分)

-- 2. _shift_seg_hours 3 參數版(p_rest 非 null → 用手動休息;null → 原階梯公式)
CREATE OR REPLACE FUNCTION public._shift_seg_hours(s time without time zone, e time without time zone, p_rest integer)
RETURNS numeric LANGUAGE sql IMMUTABLE AS $function$
  WITH x AS (
    SELECT CASE WHEN s IS NULL OR e IS NULL THEN NULL::numeric
      ELSE (EXTRACT(EPOCH FROM e) - EXTRACT(EPOCH FROM s))/60.0
           + CASE WHEN e <= s THEN 1440 ELSE 0 END  -- 跨午夜
      END AS m
  )
  SELECT COALESCE(
    GREATEST(0, m - CASE WHEN p_rest IS NOT NULL THEN p_rest
                         WHEN m >= 540 THEN 60 WHEN m >= 300 THEN 30 ELSE 0 END) / 60.0,
    0)
  FROM x
$function$;

-- 3. 班表工時加總:主段吃 rest_minutes、第二段(兩頭班另一半)走公式
CREATE OR REPLACE FUNCTION public._scheduled_work_hours(p_emp_id integer, p_period text)
RETURNS numeric LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $function$
  SELECT COALESCE(SUM(
      public._shift_seg_hours(sc.actual_start,   sc.actual_end,   sc.rest_minutes)
    + public._shift_seg_hours(sc.actual_start_2, sc.actual_end_2, NULL::integer)
  ), 0)
  FROM public.schedules sc
  WHERE sc.employee_id = p_emp_id
    AND sc.leave_request_id IS NULL           -- 請假日走請假,不重複給
    AND to_char(sc.date, 'YYYY-MM') = p_period
$function$;

-- 4. 計薪引擎:封頂/班表工時 3 處改傳 s.rest_minutes(見下方 CREATE OR REPLACE,自動生成)
