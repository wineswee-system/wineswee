-- 補記「誰簽的」— web 簽 HR A(請假/加班/出差/補打卡)中間關 — 2026-07-21
-- ════════════════════════════════════════════════════════════════════════════
-- web_advance_chain_request 每個分支都有設 approved_by=emp.name(駁回/最後關/終態關/無鏈),
--   ★唯獨「中間關推進」那行漏了★ → ASH trigger 抓不到簽核人 → web 簽的中間關 approver=NULL。
-- LIFF 路徑(liff_approve_request)中間關本來就有設 approved_by → 是 web 這行與 LIFF 不一致。
-- 修:中間關 UPDATE 補上 approved_by=emp.name(唯一改動,逐字重現 live 定義,diff 驗證)。
-- 註:歷史已簽中間關簽核人已遺失無法回填;往後 web 簽即有軌跡。
-- ════════════════════════════════════════════════════════════════════════════
