-- 資遣費平均工資修正 — 2026-07-21
-- Bug:calc_severance 平均工資讀 payroll_records.gross(含加班,且部分人=0) + salary_structures.base
--   (約45%=0)→ 兩者皆空的人(如陳虹,真實薪資在 salary_records=85000)算成 0 → 資遣費 $0。
-- 修:平均工資改「本薪+固定津貼(主管/伙食/交通),不含加班」(對齊特休折現基準);
--   來源優先 salary_records(已發布實薪,最可靠 169筆) → salary_structures → employees 主檔。
-- 其餘公式(年資/資遣月數 0.5月封頂6/預告天數/預告工資/回傳)一字不動。
