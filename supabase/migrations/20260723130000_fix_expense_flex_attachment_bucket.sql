-- 修 非費用/費用申請 LINE 卡片附件連結缺 bucket 名 — 2026-07-23
-- ────────────────────────────────────────────────────────────────────────────
-- 病灶：_push_expense_request_flex 附件連結硬拼 v_storage_base || storage_path,
--   而 storage_path 是 'expense-requests/{id}/...'(不含 bucket),檔案實際在 bucket
--   'attachments' 裡。少了 'attachments/' → URL 變 .../public/expense-requests/...
--   → Supabase 回 400「Bucket not found」→ 審核人點卡片附件開出來是錯誤頁(誤以為亂碼)。
--   LIFF 正常是因為它用 supabase-js getPublicUrl('attachments', path) 會自動帶 bucket。
-- 實測：缺bucket→HTTP400; 帶 attachments/→HTTP200 application/pdf。
-- 修法：附件 uri 改 v_storage_base || 'attachments/' || storage_path。只改這一行。
-- 基底：pg_get_functiondef 抓的【live】定義(已比對無漂移,老闆近期migration未動此函式),
--   逐行忠實還原後僅替換 uri 那行,不憑印象重貼。idempotent(CREATE OR REPLACE)。

CREATE OR REPLACE FUNCTION public._push_expense_request_flex(p_line_user_id text, p_liff_id text, p_request_id integer, p_event text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
    ...
    v_att_block := v_att_block || jsonb_build_array(
      jsonb_build_object(
        ...
        'action',     jsonb_build_object(
                        ...
                        'uri',   v_storage_base || 'attachments/' || v_att_rec.storage_path
                      )
      )
    );
  END LOOP;
