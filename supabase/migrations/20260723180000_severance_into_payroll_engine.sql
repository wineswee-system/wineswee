-- 資遣金入當月薪資:計薪引擎 _compute_payroll_for_employee — 2026-07-23
-- ════════════════════════════════════════════════════════════════════════════
-- 需求:資遣金要進離職當月薪資單(試算+入帳)。引擎是試算/入帳單一來源,故改此處。
-- 做法(diff 驗證只動 4 段):
--   1) DECLARE v_sev_amount/notice/total/rec_id
--   2) B3:離職當月(v_is_final)抓 severance_records(termination_date 落在當月,排除取消/退回),
--      取 severance_amount+notice_wage=v_sev_total。★特休折現「不」重複加(B2 v_unused_payout 已算)。
--   3) v_gross += v_sev_total → 淨額 558 自動含。★nhi 補充保費(520-543)看特定所得類別非 gross,
--      故資遣金「不」被課二代健保;勞健保看投保級距、所得稅本不代扣 → 資遣金原額入帳不被扣。
--   4) RETURN 加 severance_amount/notice_wage/total/record_id 供 generate_payroll 寫欄。
