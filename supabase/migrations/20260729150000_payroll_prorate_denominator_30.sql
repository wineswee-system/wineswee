-- 月薪部分工時比例:分母固定 30(不再用當月曆日)— 2026-07-29
-- ════════════════════════════════════════════════════════════════════════════
-- 需求:離職/新到職當月的月薪按比例 = 在職天數 / 30(固定),不用當月曆日(28/30/31)。
--   例:7/11 到職 → 在職 7/11~7/31 = 21 天 → 21/30 = 70%。
-- 只改 _compute_payroll_for_employee 兩處(逐字對、其餘完全不動):
--   1. v_sal_ratio := v_sal_actual / v_total_days  →  / 30   (真正算錢的月薪比例)
--   2. 輸出 salary_total_wd: v_total_days → 30                (前端 badge 顯示分母)
-- 不動:v_sal_actual(分子=在職天數不變)、特休折現日薪(仍 /當月曆日)、投保 proration。
-- preview_payroll 與 generate_payroll 都 delegate 此引擎 → 試算與入帳一致。
-- ════════════════════════════════════════════════════════════════════════════
