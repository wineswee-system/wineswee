-- 資遣費/最後結算:當月離職也算 final(不必等 status 翻成離職)— 2026-07-29
-- ════════════════════════════════════════════════════════════════════════════
-- 病灶:引擎 v_is_final := (status='離職')。月底離職者(如 7/31 離職、今天 7/29)status
--   還是「在職」→ v_is_final=false → 資遣費/特休折現當月不進薪資。
--   3 人資遣只有蔡沛潔(7/21已離職→status=離職)進,孫嘉澤/吳昕芛(7/31)沒進。
-- 修:v_is_final 也認「resign_date 落在計薪月內」→ 當月就是最後結算月,資遣費+特休折現進。
--   月底離職者仍領全月月薪(proration eff_end=月底→比例1),加資遣費,正確。
-- 只改 _compute_payroll_for_employee 這一處(其餘含前一支的 /30 分母都保留、逐字對)。
-- ════════════════════════════════════════════════════════════════════════════
