-- 離職當月:未過期補休也全結清折現(不只兌現已過期)— 2026-07-29
-- ════════════════════════════════════════════════════════════════════════════
-- 病灶:引擎補休兌現只抓 status='active' AND expires_at < 月底(已過期)。離職者未過期的
--   補休不會結清 → 離職未休補休沒折現(勞基法應折現)。
-- 修:_compute_payroll_for_employee 補休 WHERE 加「離職當月」分支 → 未過期補休也全結清。
--   離職者無下個月可重複兌現,故安全(不需另標 ledger settled 也不會雙付)。
-- 只改這一句(前面 /30 分母、當月離職算final 逐字保留)。
-- ════════════════════════════════════════════════════════════════════════════
