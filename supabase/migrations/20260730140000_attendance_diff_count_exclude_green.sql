-- 打卡核對報表:差異筆數排除「多上時數」 — 2026-07-30
-- ════════════════════════════════════════════════════════════════════════════
-- 需求:差異筆數(diff_count)只算「需處理」的差異 = 遲到 LATE / 時數不足 UNDERTIME /
--   未打卡 MISSING / 早退 EARLY_LEAVE / 未排班打卡 UNSCHEDULED。
--   只有「多上時數 OVERWORK」(排班日上超過時數,屬正向)不算差異。
-- 只改 admin_attendance_diff_report 的 total(彙總計數);type_counts 仍回全部類型(篩選下拉用),
--   詳細(monthly_attendance_diff)不動,OVERWORK 項目仍會列出(色碼標示、僅供參考)。
-- ════════════════════════════════════════════════════════════════════════════
