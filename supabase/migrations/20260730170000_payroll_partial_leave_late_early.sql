-- 部分請假日的遲到/早退(step 3)— 2026-07-30
-- ════════════════════════════════════════════════════════════════════════════
-- 規格 project_partial_leave_break_spec。重建 _compute_payroll_for_employee,
-- 只改「遲到/早退」那段:部分請假(時數假)當天不再整天豁免,改用調整界線判
--   (蓋到上班→應到=請假結束;蓋到下班→應走=請假起始;容 grace;行政該日不吃浮動)。
-- ★ 無請假日 ast/ae 與原式逐字一致 → 沒請假的人薪水完全不變。
-- ★ 全天假(start_time IS NULL)仍整天豁免。
-- ★ 本檔由程式讀 live dump(2026-07-30 08:51)精準替換該段產生,其餘 byte 不動。
-- 已知小限制:同一天有多筆時數假只取最早一筆調整(罕見)。
-- 驗收:跑 preview_payroll 對 4 案例(行政3/門市早退/門市滿班/反例)確認後才入帳。
-- ════════════════════════════════════════════════════════════════════════════
...
DECLARE
  ...
  -- 資遣金入薪(離職當月接 severance_records:資遣金+預告工資;特休折現由 B2 v_unused_payout 處理,不重複)
...
BEGIN
  ...
  FROM (
    SELECT
      CASE
        WHEN ar.clock_in IS NOT NULL AND ar.clock_out IS NOT NULL THEN
          -- 封頂在班表:打卡只抓遲到早退;打卡>班表(飄移/未核准超時)不計本薪,超時要走加班申請(另計);打卡<班表→用打卡照扣
          CASE WHEN s.actual_start IS NOT NULL AND s.actual_end IS NOT NULL
               THEN LEAST(COALESCE(ar.total_hours,0), public._shift_seg_hours(s.actual_start, s.actual_end, s.rest_minutes))
               ELSE COALESCE(ar.total_hours,0) END
        WHEN (ar.clock_in IS NULL) <> (ar.clock_out IS NULL)
          ...
          THEN public._shift_seg_hours(s.actual_start, s.actual_end, s.rest_minutes)
  ...
  -- 天災照給薪:時薪員工當天有排班(非休假)、沒打卡(沒來)、且該日在天災(paid)宣告涵蓋其門市 → 補班表時數當已給薪
  IF v_is_hourly THEN
    SELECT COALESCE(SUM(public._shift_seg_hours(s.actual_start, s.actual_end, s.rest_minutes)), 0)
      INTO v_disaster_hours
    FROM public.schedules s
    WHERE s.employee_id = p_emp_id
      AND s.date >= v_effd_start AND s.date <= v_effd_end
      AND s.actual_start IS NOT NULL AND s.actual_end IS NOT NULL AND s.absence_type IS NULL
      AND EXISTS (
        SELECT 1 FROM public.disaster_days dd
         WHERE dd.no_show_handling = 'paid'
           AND s.date >= COALESCE(dd.start_at::date, dd.date)
           AND s.date <= COALESCE(dd.end_at::date, dd.date)
           AND (dd.store_ids IS NULL OR COALESCE(array_length(dd.store_ids,1),0)=0 OR v_emp.store_id = ANY(dd.store_ids)))
      AND NOT EXISTS (
        SELECT 1 FROM public.attendance_records ar
         WHERE ar.employee_id = p_emp_id AND ar.date = s.date AND ar.clock_in IS NOT NULL);
    v_hours := v_hours + v_disaster_hours;
  END IF;

  -- ── 遲到/早退（per-minute = 分鐘 × 時薪/60；計件不扣）──
  --   有班表(actual_start/end)用班表；admin 沒班表退回固定 09:00–18:00。grace：admin 30、其他 0。
  --   ★ 部分請假(時數假)當天:不再整天豁免,改用調整過的界線判——
  --     請假蓋到上班(容 grace)→ 應到=請假結束;蓋到下班(容 grace)→ 應走=請假起始;
  --     且該日行政「不吃 clock_in+span 浮動」,應下班改用固定辦公結束(避免浮動把應走推很晚)。
  --   全天假仍整天豁免(下方 NOT EXISTS 只排除 start_time IS NULL 的全天假)。
  --   無請假日 ast/ae 與原式完全一致(沒請假的人薪水不變)。跨午夜+1440;早打卡不 credit。
  IF NOT v_is_piece THEN
    ...
    FROM (
      SELECT
        ...
        -- 應上班:時數假蓋到上班(容 grace)→ 應到=請假結束;否則班表/辦公起始(=原式)
        CASE
          WHEN lv.lst_min IS NOT NULL
               AND lv.lst_min <= COALESCE(EXTRACT(EPOCH FROM s.actual_start::time)/60.0,
                                          CASE WHEN COALESCE(v_emp_category,'')='admin' THEN v_start_base END)
                                 + (CASE WHEN COALESCE(v_emp_category,'')='admin' THEN v_admin_grace ELSE 0 END)
          THEN lv.let_min
          ELSE COALESCE(EXTRACT(EPOCH FROM s.actual_start::time)/60.0,
                        CASE WHEN COALESCE(v_emp_category,'')='admin' THEN v_start_base ELSE NULL END)
        END AS ast,
        -- 應下班:
        CASE
          WHEN lv.lst_min IS NOT NULL THEN
            CASE
              WHEN lv.let_min >= COALESCE(EXTRACT(EPOCH FROM s.actual_end::time)/60.0,
                                          CASE WHEN COALESCE(v_emp_category,'')='admin' THEN v_end_base END)
                                 - (CASE WHEN COALESCE(v_emp_category,'')='admin' THEN v_admin_grace ELSE 0 END)
              THEN lv.lst_min
              ELSE COALESCE(EXTRACT(EPOCH FROM s.actual_end::time)/60.0,
                            CASE WHEN COALESCE(v_emp_category,'')='admin' THEN v_end_base ELSE NULL END)
            END
          ELSE
      ...
      LEFT JOIN LATERAL (
        SELECT EXTRACT(EPOCH FROM lr.start_time::time)/60.0 AS lst_min,
               EXTRACT(EPOCH FROM lr.end_time::time)/60.0
                 + CASE WHEN lr.end_time <= lr.start_time THEN 1440 ELSE 0 END AS let_min
        FROM leave_requests lr
        WHERE lr.employee_id = ar.employee_id AND lr.status = '已核准' AND lr.deleted_at IS NULL
          AND lr.start_time IS NOT NULL
          AND ar.date BETWEEN lr.start_date AND COALESCE(lr.end_date, lr.start_date)
        ORDER BY lr.start_time LIMIT 1
      ) lv ON true
      WHERE ar.employee_id = p_emp_id AND ar.date >= v_effd_start AND ar.date <= v_effd_end
        ...
        -- 只排除「全天假」(start_time IS NULL);時數假改用上面調整過的 ast/ae 判遲到早退
        AND NOT EXISTS (
  ...
  WHERE employee_id=p_emp_id AND status='active'
    AND (expires_at < v_mend
         OR v_emp.status = '離職'
         OR (v_emp.resign_date IS NOT NULL AND v_emp.resign_date >= v_mstart AND v_emp.resign_date <= v_mend));
  ...
  IF NOT v_is_hourly THEN
    ...
    IF v_eff_start > v_mstart OR v_eff_end < v_mend THEN
      ...
      v_sal_ratio  := v_sal_actual::numeric / 30;   -- 分母固定30(不用當月曆日)— 2026-07-29
    END IF;
  END IF;
  ...
  IF v_is_final THEN
    IF v_is_hourly THEN
      ...
      WHERE employee_id = p_emp_id AND year = v_year
        ...
        AND (period_start IS NULL OR period_start <= v_mend);
      v_unused_payout := ceil(v_unused_days * v_hourly_rate * 8);
    ELSE
      -- 月薪:特休未休折現 = 年資現算天數 × 平均工資日薪(本薪+固定津貼)/30
      DECLARE
        v_sen         numeric;
        v_annual_ent  numeric;
        v_annual_used numeric;
        v_avg_daily   numeric;
      BEGIN
        v_sen := (COALESCE(v_emp.resign_date::date, v_mend) - v_emp.join_date::date) / 365.25;
        v_annual_ent := CASE
          WHEN v_emp.join_date IS NULL OR v_sen < 0.5 THEN 0
          WHEN v_sen < 1  THEN 3
          WHEN v_sen < 2  THEN 7
          WHEN v_sen < 3  THEN 10
          WHEN v_sen < 5  THEN 14
          WHEN v_sen < 10 THEN 15
          ELSE LEAST(30, 15 + floor(v_sen - 10)::int)
        END;
        SELECT COALESCE(SUM(used_days),0) INTO v_annual_used
          FROM leave_balances
         WHERE employee_id = p_emp_id AND year = v_year
           AND leave_type IN ('特休','annual','特別休假');
        v_unused_days := GREATEST(v_annual_ent - v_annual_used, 0);
        v_avg_daily := (v_base_salary
          + COALESCE(v_ss.meal_allowance,0)      + COALESCE(v_ss.role_allowance,0)
          + COALESCE(v_ss.supervisor_allowance,0) + COALESCE(v_ss.night_shift_allowance,0)
          + COALESCE(v_ss.cross_store_allowance,0) + COALESCE(v_ss.transport_allowance,0)
          ) / NULLIF(v_total_days,0);
        v_unused_payout := ceil(v_unused_days * v_avg_daily);
      END;
    END IF;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'disasterHours', v_disaster_hours,
