-- WEB 待簽核:希望休過濾對齊 LIFF(改走組織圖簽核人)— 2026-08-03
-- ════════════════════════════════════════════════════════════════════════════
-- 症狀:店長送希望休已修 LIFF(_resolve_hr_approver_ids 只回直屬主管),但 WEB 簽核中心
--   仍讓「有 schedule.approve 權限的人」全部看得到 → admin/super_admin 照樣看到全 org
--   待審希望休。根因:web_list_my_pending_approval_ids 的 off_requests 分支用「該單門市
--   店長 OR schedule.approve 權限」權限制,沒吃組織圖 _resolve_hr_approver_ids。
-- 修:off_requests 分支改為 emp.id IN (_resolve_hr_approver_ids(orr.employee_id)),
--   與 LIFF liff_list_pending_approvals 一致 → WEB 也只剩直屬主管(張庭瑋)看得到。
-- 其餘內容逐字保留(程式單點置換,非重寫)。idempotent。
-- ════════════════════════════════════════════════════════════════════════════
