-- 計薪引擎:編制內行政「曠職」自動扣款 — 2026-08-03
-- ════════════════════════════════════════════════════════════════════════════
-- 需求:行政(無班表)應上班卻「整天沒打卡、又沒請假」→ 視為曠職,扣 1 日(base_for_ins/30)。
-- 規則(老闆定案):
--   對象 = employment_category='admin' 且 employees.in_payroll=true(編制外免打卡,不列曠職)。
--   應上班日 = 週一~五 且 非國定假(holidays.is_workday=false) 且 非天災(disaster_days,依門市)。
--   曠職 = 應上班日 且 當天無打卡(attendance_records.clock_in) 且 無核准請假 且 無核准出差。
--   扣款 = 曠職天 × 8h × 時薪(=base_for_ins/30),與無薪假同率;併入 v_absence_deduct 進 v_total_deduct。
--   補救:忘打卡但有上班者,補打卡核准 → 產生 attendance_records → 自動不算曠職。
-- 唯讀試算(2026-07)已對帳:編制內行政真實曠職=張開翔07-31、蔡沛潔07-21,共 2 人 2,867。
--   天災 07-10(總部 paid)、編制外(韓德森等)已正確排除。
-- 手法:dump live def 後 4 處精準插入(DECLARE/計算/扣款/回傳),其餘逐字保留,非重寫。idempotent。
-- ════════════════════════════════════════════════════════════════════════════
