-- LIFF 人力儀表板:今日加班套換日歸屬(00:00~ 跨夜尾段不算今天)— 2026-08-03
-- ════════════════════════════════════════════════════════════════════════════
-- 對齊 web TeamDashboard 修正:liff_hr_dashboard 的「今日特殊狀態」加班(已核准+待審)
-- 原用 o.date = v_today 原始比對 → 00:00~01:00 的跨夜加班(照 6 點換日應歸前一天)被誤算今天。
-- 改:start_time < 06:00 → 歸前一天再比 v_today。純顯示歸類,不動金額。
-- dump live 後置換 2 處(已核准/待審加班),其餘逐字保留。idempotent。
-- ════════════════════════════════════════════════════════════════════════════
