-- 修:計薪引擎還原資遣費(B3)+ 特休折現生效日閘門 — 2026-08-03
-- ============================================================================
-- 兩件被後續引擎重寫洗掉/沒判的事,一起補回 _compute_payroll_for_employee:
-- ② 資遣費(B3):某次重寫把「離職當月抓 severance_records 加進 gross + 回傳 severance 欄」
--    整段弄丟 → 引擎/generate 都沒寫資遣費 → 批次計薪「資遣費」欄永遠 '-'。這裡照
--    20260723180000 原樣還原:離職(v_is_final)抓 termination 落在當月、未取消/退回的
--    severance_records,severance_amount+notice_wage 加進 gross,並回傳 4 個 severance 欄。
-- ③ 特休折現閘門:B2 離職結清原本只要離職就折現特休,沒判「特休生效日=到職+6個月」→
--    未滿6個月離職者(如鄭力瑄 12/01 才生效、404/111/430)被誤兌現。加閘門:到職+6月<=離職日才折現。
-- 影響:試算 preview 與入帳 generate_payroll 同步(generate 用引擎回傳值)。
-- 做法:DB dump 現行引擎後「單點取代」4 處,其餘 byte 不動。idempotent。
-- 驗收:preview_payroll('2026-07') → 111/430/404/鄭力瑄 特休折現=0、136 維持 3581;
--       406 吳昕芛出現 severance_total=4180、gross 多 4180。
-- ============================================================================
...
BEGIN
  ...
  IF COALESCE(v_emp_category,'') = 'admin' AND COALESCE(v_emp.in_payroll, false) = true THEN
  ...
  RETURN jsonb_build_object(
    ...
    '_awol_rows', COALESCE((
      ...
      WHERE COALESCE(v_emp_category,'') = 'admin' AND COALESCE(v_emp.in_payroll, false) = true
        ...
        AND NOT EXISTS (SELECT 1 FROM leave_requests lr WHERE lr.employee_id = p_emp_id AND lr.status = '已核准' AND g.d::date BETWEEN lr.start_date AND COALESCE(lr.end_date, lr.start_date))
