-- 修:曠職自動扣款擴到門市(不再只有行政)— 2026-08-03
-- ============================================================================
-- 需求:門市正職「當天有排上班班別、卻整天沒打卡、又沒請假/出差」→ 視為曠職自動扣 1 日,
--   不用再手動天災結算。應上班日:admin=週一~五非國定;門市=當天排了上班班別(schedules
--   actual_start,非休/例假/補休)。共用:非天災、無打卡、無核准請假、無核准出差。
-- 只對月薪生效(兼職時薪沒打卡本來就 0,deduction 已 guard v_is_hourly)。
-- 天災日仍排除 → 天災不支薪走「沒來結算」產無薪假,不與曠職重複。
-- 安全網:真的有上班忘打卡 → 補打卡核准 → 產生 attendance_records → 自動不算曠職。
-- ⚠️ 全門市計薪行為改變、動到錢:上線前務必先跑唯讀 preview_payroll 檢視每人 awolDays/awolDeduction,
--    確認沒有因打卡資料缺漏而誤判大量曠職,再入帳。DB dump 後單點取代 awol 區塊。idempotent。
-- ============================================================================
