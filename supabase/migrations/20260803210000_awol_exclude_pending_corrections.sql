-- 修:曠職(awol)排除「已申請補打卡(待審核/已核准)」的日子 — 2026-08-03
-- ============================================================================
-- 病灶:awol 只看有無打卡,沒看補打卡申請 → 一堆人正在申請補卡(待審核)卻被先當曠職扣。
--   實例:趙亭威 11 天曠職,其實那 11 天全都有補卡申請、狀態全「待審核」。全公司 7 月
--   共 222 筆待審核補卡。核准後才會產生打卡紀錄,在那之前不該預先扣。
-- 修:awol 條件再加一條 —— 當天有「未被駁回」的補打卡申請(clock_corrections,status<>已駁回、
--   未刪)→ 不算曠職。只有「沒打卡且沒補卡申請(或補卡被駁回)且沒請假出差」才是真曠職。
-- DB dump 後單點取代(awol WHERE 尾端加一條 NOT EXISTS)。idempotent。
-- ============================================================================
...
BEGIN
  ...
  IF COALESCE(v_emp.in_payroll, false) = true AND NOT v_is_hourly THEN
    ...
    WHERE (CASE WHEN COALESCE(v_emp_category,'') = 'admin'
      ...
      AND NOT EXISTS (SELECT 1 FROM clock_corrections cc
                       WHERE cc.employee_id = p_emp_id AND cc.date = g.d::date
                         AND COALESCE(cc.status,'') <> '已駁回' AND cc.deleted_at IS NULL);
  END IF;
