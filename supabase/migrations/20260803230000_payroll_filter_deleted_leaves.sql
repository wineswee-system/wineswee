-- 修:計薪引擎 leave_requests 查詢補 deleted_at IS NULL(soft_delete 慘案)— 2026-08-03
-- ============================================================================
-- 病灶:引擎所有 leave_requests 查詢沒濾 deleted_at → 被軟刪的假單仍計薪扣款、明細顯示,
--   NOT EXISTS 還誤把該天當「請假中」漏算曠職/遲到早退。實例:天災沒排班誤建、已軟刪的
--   無薪假(劉家君 id287 7/10)在請假管理看不到,計薪卻照扣/照顯示(??? 那張圖)。
-- 修:8 處 leave 查詢全補 deleted_at:遲到早退淨工時排除(174)、遲到早退明細排除(226)、
--   請假聚合扣款(355)、awol排除(379)、_leave_rows明細(700)、awol明細排除(710)、
--   遲到明細排除(727)、早退明細排除(747)。★加班 overtime_requests 的 3 處未動(另議)。
-- DB dump 後精準 replace;idempotent。驗:劉家君 7/10 無薪假不再入扣款/明細。
-- ============================================================================
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    '_awol_rows', COALESCE((
      ...
      FROM generate_series(v_effd_start::timestamp, v_effd_end::timestamp, interval '1 day') g(d)
      WHERE COALESCE(v_emp_category,'') = 'admin' AND COALESCE(v_emp.in_payroll, false) = true
