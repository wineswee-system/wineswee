-- 修:曠職明細列(_awol_rows)擴到門市,顯示曠職日期(不再只有行政)— 2026-08-03
-- ============================================================================
-- 病灶:曠職「扣款」已擴門市(190000),但曠職「明細列」_awol_rows 的 gate 還是 admin-only
--   → 門市有曠職天數卻列不出日期(UI 曠職日期顯示「—」)。
-- 修:把 _awol_rows 的 gate 改成跟扣款同一套 —— admin=週一~五非國定;門市=當天排上班班別。
--   其餘(disaster/attendance/leave(已含deleted_at)/trip 排除)不動。DB dump 後單點取代。
-- ============================================================================
