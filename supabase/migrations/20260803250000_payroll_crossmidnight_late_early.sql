-- 修:跨午夜班遲到早退 — 午夜後打卡(<6:00)算班表尾巴 +1440 — 2026-08-03
-- ============================================================================
-- 病灶:遲到早退公式對跨午夜班(actual_end<=actual_start)的「應下班 ae」已 +1440,但 clock_in/
--   clock_out 只在「下班<上班」時才 +1440。當 in=out 都在午夜後(吳昕芛 7/25 班15:00-00:00 刷 00:11)
--   → cot 停在 11 → 早退=1440-11=1429(扣4366)完全算反。
-- 修:跨午夜班時,凡打卡在 06:00 前(EPOCH<21600)一律 +1440(ci/cot 都是),對齊班表尾巴。
--   7/25:cot 00:11→1451,早退=0;ci 00:11→1451,遲到=1451-900=551(正確)。
--   正常跨午夜(上班14:56/下班00:05):14:56 不<06:00 不動;00:05 本就 +1440 → 結果不變。
-- 改 6 處:主計算 cot 1、早退明細 cot 1、ci 3、(遲到明細 ci 已含在 3)。DB dump 後精準 replace。
-- ============================================================================
...
BEGIN
  ...
  FROM (
    ...
    WHERE employee_id = p_emp_id AND status='已核准' AND NOT COALESCE(is_comp_leave,false)
    ...
    '_ot_rows', COALESCE((
      ...
      FROM (
        SELECT request_date, cat, SUM(ot_hours) AS reg_h FROM (
          ...
          FROM overtime_requests WHERE employee_id=p_emp_id AND status='已核准' AND NOT COALESCE(is_comp_leave,false) AND NOT COALESCE(is_exception,false)
            AND request_date>=v_effd_start AND request_date<=v_effd_end
    ...
    '_ot_exception_rows', COALESCE((
      ...
      FROM (
        ...
        FROM (
          ...
          FROM overtime_requests WHERE employee_id=p_emp_id AND status='已核准' AND NOT COALESCE(is_comp_leave,false)
            AND request_date>=v_effd_start AND request_date<=v_effd_end
