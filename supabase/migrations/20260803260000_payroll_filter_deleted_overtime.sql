-- 修:計薪引擎 overtime_requests 查詢補 deleted_at IS NULL(潛在 soft_delete 雷)— 2026-08-03
-- ============================================================================
-- 承 leave 版(230000)。引擎 4 處 overtime_requests 查詢(加班費/加班時數聚合+明細)都沒濾
-- deleted_at → 若已核准加班單被軟刪,仍會被算加班費(目前實測 0 筆已核准+軟刪,無現行影響,
-- 屬預防性)。4 處共用 "AND NOT COALESCE(is_comp_leave,false)",一律接 "AND deleted_at IS NULL"。
-- DB dump 後 replace_all(4處);idempotent。★是否也要濾 is_exception 等另議,本次只補 deleted_at。
-- ============================================================================
