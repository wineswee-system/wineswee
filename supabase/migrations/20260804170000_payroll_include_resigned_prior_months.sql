-- 計薪員工範圍:離職者仍納入「離職前的月份」+ 結算閘門收緊到離職當月 — 2026-08-04
-- ════════════════════════════════════════════════════════════════════════════
-- 症狀:員工於 M 月離職後,狀態變離職 → 計薪/薪資頁對「M 月之前的月份」把他排除
--   (舊篩選:離職且 resign_date 落在該月),導致他「上班了卻領不到那些月薪水」。
--   實例:松江長安 孫嘉澤(#131)、陳宜彬(#441)皆 2026-08-03 離職 → 看 7 月時消失,
--   但兩人整個 7 月都在職、7 月薪水未記。(其他店沒人在 7 月之後離職,故沒暴露。)
--
-- 修法(4 處,3 支 DB + 1 前端):
--   篩選改為「該月有在職」= 到職≤月底 AND (在職 OR resign_date≥月初)
--     → 離職者在「到職~離職」之間的每個月都算得到;離職後的月份仍排除。
--   引擎 _compute 的 v_is_final(特休折現/資遣費)閘門:status=離職 → 收緊成「離職當月」
--     (EXTRACT year/month = 該計薪月)→ 放寬篩選後,離職前月份不會被塞折現/資遣費(防重複給付)。
--
--   1) preview_payroll   篩選 + 既有 source_store 門市回補
--   2) generate_payroll  篩選(拿掉 resign<=月底 上界)
--   3) _compute_payroll_for_employee  v_is_final 閘門收緊
--   4) 前端 Salary.jsx 撈員工篩選(另一支 commit)
--
-- 相容:入帳(generate)/試算(preview)/引擎 同源;離職當月行為完全不變(折現/資遣照舊)。
-- ════════════════════════════════════════════════════════════════════════════

-- ── (1) preview_payroll:篩選放寬 + 離職者 source_store 門市回補 ──
...
-- ── (2) generate_payroll:篩選放寬(拿掉 resign<=月底 上界)──
...
-- ── (3) _compute_payroll_for_employee:v_is_final 閘門收緊到離職當月 ──
