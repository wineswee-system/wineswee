-- 還原補卡「6 點換天」— live 漂移把 boundary 洗掉了 — 2026-08-04
-- ════════════════════════════════════════════════════════════════════════════
-- 病灶:_apply_correction_to_attendance 的 live 版本掉了「凌晨6點前歸前一天」邏輯
--   (repo 最終版 20260715140000 有 v_att_date/v_boundary,live 卻直接用 c.date)。
--   → 夜班 00:00~06:00 的下班補卡照日曆日建紀錄,生出「無上班、只有下班」的孤兒
--   (如李欣霏 #370 下班填8/4 00:00 → 8/4 孤兒 #4871,本該滾回8/3那班)。全公司22筆。
-- 成因:repo 有 715 兩支,live 無 → Studio hotfix 用舊底重建函式時洗掉 boundary(漂移)。
-- 修法:逐字重貼 20260715140000 版(live = 此版 − boundary 段,純還原、不減任何現有功能:
--   模式夾值/中文type相容/扣休息重算工時 全保留)。只修「未來」補卡,不動歷史22筆孤兒。
-- boundary 讀 organizations.settings.day_boundary_hour(預設6)。idempotent。
-- ════════════════════════════════════════════════════════════════════════════
