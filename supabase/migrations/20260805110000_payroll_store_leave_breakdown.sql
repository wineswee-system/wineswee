-- 已存薪資單存請假明細(逐筆假別)給 PayslipRow 用 — 2026-08-05
-- ════════════════════════════════════════════════════════════════════════════
-- payroll_records 加 leave_breakdown jsonb;generate_payroll 存檔時把引擎已算好的
--   _leave_rows([{date,type,hours,days}])一起存(★只存不算,不動任何金額欄)。
--   dump live 版單點 append 到 INSERT 欄位列+VALUES 最後(零錯位),其餘逐字不動。
-- ════════════════════════════════════════════════════════════════════════════
ALTER TABLE public.payroll_records ADD COLUMN IF NOT EXISTS leave_breakdown jsonb DEFAULT '[]'::jsonb;
