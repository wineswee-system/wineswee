-- 計薪引擎:凌晨加班套「6點換日」判類別(對齊打卡追蹤)— 2026-08-05
-- ════════════════════════════════════════════════════════════════════════════
-- 病灶:加班單 request_date 存日曆日(如 00:00~00:30 存 7/3),引擎用 request_date 判類別
--   → 7/3 是例假就判例假 ×1.0,但那其實是 7/2 夜班尾巴該判平日 ×1.34。打卡追蹤(Attendance)
--   早有套「start<6點→前一天」,只有引擎沒套 → 兩邊對不上、加班費算錯(7月17筆,雙向)。
-- 修法:引擎 4 個 OT 子查詢的 request_date 改用 eff_date=
--   (CASE WHEN start_time < TIME '06:00' THEN request_date-1 ELSE request_date END),
--   判類別+分組+逐筆明細日期都用它(NULL start_time 不滾,對齊 Attendance)。
--   dump live 版單點替換 4 處,其餘逐字不動。★只改「哪天/哪類」,金額公式沒動。
--   期間篩選(request_date>=v_effd_start)仍用原始日,不跨月搬動加班單。
-- ════════════════════════════════════════════════════════════════════════════
