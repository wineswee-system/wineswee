-- 整天請假扣款/明細改用「當天實際排班淨時數」(取代假設8h)— 2026-08-05
-- ════════════════════════════════════════════════════════════════════════════
-- 需求:整天無薪/事/病假時數不該一律8h,要用當天實際排班。原 COALESCE(hours,days*8) 全日固定8h。
-- 新 helper _scheduled_net_hours(emp,date)=班窗−休息(_shift_break_context;跨午夜+24;兩頭班段間算休息)。
-- 引擎請假 SUM + _leave_rows 改 COALESCE(hours, 排班淨時數, days*8):填時數用填的;整天用實排;無班表退8h。
-- ★動到無薪/半薪假實際扣款。dump live 單點替換2段,其餘金額公式沒動。
-- ════════════════════════════════════════════════════════════════════════════
CREATE OR REPLACE FUNCTION public._scheduled_net_hours(p_emp_id integer, p_date date)
 RETURNS numeric LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $function$
DECLARE ctx record; v_span numeric; v_break numeric;
BEGIN
  SELECT * INTO ctx FROM public._shift_break_context(p_emp_id, p_date);
  IF ctx.shift_start IS NULL THEN RETURN NULL; END IF;
  v_span := EXTRACT(EPOCH FROM (ctx.shift_end - ctx.shift_start)) / 3600.0;
  IF v_span < 0 THEN v_span := v_span + 24; END IF;
  v_break := CASE WHEN ctx.break_start IS NOT NULL AND ctx.break_end IS NOT NULL
                  THEN EXTRACT(EPOCH FROM (ctx.break_end - ctx.break_start)) / 3600.0 ELSE 0 END;
  RETURN ROUND(GREATEST(v_span - v_break, 0)::numeric, 2);
END $function$;
