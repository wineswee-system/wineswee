-- 留職停薪(育嬰留停)接進計薪引擎:留停期間底薪+津貼歸比例、公司勞健保照付 — 2026-08-05
-- ════════════════════════════════════════════════════════════════════════════
-- 病灶:引擎薪資比例 v_sal_ratio 只看到職/離職日,不認留職停薪(留停維持在職)→ 比例=1
--   → 留停員工照發全薪(實測空月陳嘉益 net 60,645)。超發。
-- 修:v_sal_actual 扣掉「被已核准育嬰留停假覆蓋的在職天數」→ 底薪+津貼×剩餘比例(整月→0)。
--   ★保費 proration 走 v_in_service(在職)不動 → 公司負擔勞健保照付(法規:留停續保有雇主負擔)。
--   整月留停 → 底薪+津貼0 → gross<=0 → 既有夾:實領0、員工自付勞健保0(不會負)。
--   半月留停 → 半薪 − 全月保費(在保);時薪PT不走此段(v_is_hourly 分支)。
-- dump live 單點替換(宣告+比例段),其餘金額公式沒動。idempotent。
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  FROM (
    SELECT
      CASE
        WHEN ar.clock_in IS NOT NULL AND ar.clock_out IS NOT NULL THEN COALESCE(ar.total_hours,0)
