-- liff_hr_dashboard:事假(事假)從 sick 獨立成 personal — 2026-08-10
-- 需求:dashboard 今日特殊狀態,病假=sick、事假=personal(前端 HrDashboardTab 加 personal/📋黃 標籤)、其餘=leave
-- 作法:以 live 定義為底,只改 2 行 CASE(病假→sick/WHEN事假→personal/ELSE leave),CREATE OR REPLACE。idempotent。
...
BEGIN
  ...
  FROM (
    SELECT DISTINCT ON (employee_id) employee_id, name, status, prio FROM (
      SELECT l.employee_id, e.name, CASE WHEN l.type = '病假' THEN 'sick' WHEN l.type = '事假' THEN 'personal' ELSE 'leave' END AS status, 1 AS prio
      ...
      UNION ALL
      SELECT l.employee_id, e.name, CASE WHEN l.type = '病假' THEN 'sick_pending' WHEN l.type = '事假' THEN 'personal_pending' ELSE 'leave_pending' END, 2
        FROM leave_requests l JOIN employees e ON e.id = l.employee_id
