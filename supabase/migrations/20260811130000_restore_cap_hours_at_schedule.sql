-- 還原並修正「本薪工時封頂在班表」(封頂在 8-10 搬遷/後續重寫時被洗掉)
-- ─────────────────────────────────────────────────────────────
-- 症狀:時薪員工本薪 = 時薪 × 實際打卡工時(含早到/晚走)→ 早到晚走多給錢。
--       黃博湋 7月 workHours 97.61(應 94);7月共 25 位時薪員工被多給 ≥ NT$17,469。
-- 正解(老闆定調):本薪 = 時薪 × 「排班淨工時」,遲到/早退再從減項另扣(單次)。
--   兩支打卡都有 + 有班表 → eff_hours = _shift_seg_hours(班表)(不用打卡、不用 LEAST);
--   缺一支打卡(有班表、非請假)→ 一樣用班表;無班表 → 退回打卡。
--   ※不可用 LEAST(打卡,班表):早退那天會被壓成打卡值,再加減項早退扣=重複扣。
-- 修法:拿當時 live 完整定義,只改該分支,其餘(含較新 deleted_at 濾除)原樣保留。
-- 影響:改完需唯讀重跑/重新結算 7 月(25 位時薪員工本薪下修)。
-- ─────────────────────────────────────────────────────────────
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(CASE WHEN lr.type IN ('病假','sick','生理假','menstrual') THEN COALESCE(lr.hours, sn.h, COALESCE(lr.days,0)*8) ELSE 0 END),0)
  ...
  v_gross := v_eff_base + v_ot_ovt_for_net + v_policy_bonus + v_unused_payout + v_sev_total;
