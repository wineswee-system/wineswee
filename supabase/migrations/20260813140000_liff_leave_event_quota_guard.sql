-- LIFF 請假送出擋關補「事件/法定假別上限」——
-- 問題:liff_insert_leave_request 只擋補休(ledger)、特休(leave_balances/§38),
--       完全沒擋婚假/病假/事假/家庭照顧假/生理假/陪產/產檢 這種有 max_days 的假別
--       → 手機可超額/重複送出(如婚假用了3天再請7天=10>8),進待審核後才被駁回,申請人白忙+誤以為有那麼多假。
-- 修:INSERT 前加「事件假上限」檢查,吃 leave_types.max_days(對齊 web leavePolicy 的 maxDays),
--     本年度已用(待審核+已核准)+本次 > 上限 → 當場擋下並講清楚剩幾天。特休/補休維持各自既有把關。
...
BEGIN
  ...
  --   特休(annual)/補休(comp_time)已在上面各自把關,排除。本年度已用(待審核+已核准)+本次 > 上限 → 當場擋。
  IF v_code IS NOT NULL AND v_code NOT IN ('annual','comp_time') THEN
    DECLARE
      v_max  numeric;
      v_used numeric;
