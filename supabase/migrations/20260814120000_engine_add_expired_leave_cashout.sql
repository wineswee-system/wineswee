-- 2026-08-14 計薪引擎接入到期折現(read-only dry進gross;離職走v_unused_payout、編制外函式回0)。驗證:現無到期→gross不變
...
DECLARE
  ...
  v_awol_deduct    numeric:=0;   -- 曠職扣款\r
...
BEGIN
  ...
  v_dependents     := COALESCE(NULLIF((SELECT COUNT(*)::int FROM public.employee_dependents WHERE employee_id = p_emp_id AND health_ins IS TRUE), 0), v_ss.health_ins_dependents, 0);  -- 數勾加保健保的眷屬;無名單才 fallback 手填人數\r
  ...
  v_late_deduction  := floor(v_late_mins  * v_hourly_rate / 60.0);  -- per-minute,無條件捨去\r
