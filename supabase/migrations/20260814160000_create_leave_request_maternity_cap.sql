-- 2026-08-14 create_leave_request +產假情形上限(56/28/7/5)+存maternity_type;修復被誤註解的女性專屬guard
DROP FUNCTION IF EXISTS public.create_leave_request(integer, text, text, date, date, time without time zone, time without time zone, text, integer);

CREATE OR REPLACE FUNCTION public.create_leave_request(p_employee_id integer, p_type_code text, p_unit text, p_start_date date, p_end_date date, p_start_time time without time zone, p_end_time time without time zone, p_reason text DEFAULT NULL::text, p_attachment_count integer DEFAULT 0, p_maternity_type text DEFAULT NULL)
