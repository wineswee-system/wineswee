-- 2026-08-17 門市稽核草稿編輯開放 admin+稽核室(原僅稽核員本人)
...
BEGIN
  ...
  IF NOT (v_audit.auditor_id = emp.id
       OR EXISTS (SELECT 1 FROM public.roles r WHERE r.id = emp.role_id AND r.name IN ('admin','super_admin'))
