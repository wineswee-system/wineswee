-- ════════════════════════════════════════════════════════════════════════════
-- 費用申請 LIFF/LINE 核准:中間關逐關記下簽核人 — 2026-08-18
-- ════════════════════════════════════════════════════════════════════════════
-- 問題:liff_approve_request 的 expense_request 中間關只做 current_step+1,沒寫
--   approved_by → ASH trigger 抄到殘值 → 中間關簽核人記錯(例:#490 總經理關)。
--   WEB 路徑(expense_request_step_advance)中間關本來就有 approved_by=v_emp.name;
--   此處讓 LIFF/LINE 路徑對齊,兩邊一致、ASH 中間關 approver 正確。
-- 手法:抓 live 定義,只在中間關 UPDATE 加 approved_by=emp.name 一個子句,
--   其餘 353 行與線上逐字相同(避免大函式重寫)。reject / 最後一關本來就有記。
-- ════════════════════════════════════════════════════════════════════════════
