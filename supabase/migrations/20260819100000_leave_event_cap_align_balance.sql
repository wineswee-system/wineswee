-- ════════════════════════════════════════════════════════════════════════════
-- 事件假別(事假/病假/家庭照顧…)年度上限檢查:對齊餘額顯示,修雙算 — 2026-08-19
-- ════════════════════════════════════════════════════════════════════════════
-- 問題:送出閘門用 SUM(days) 且 type IN (name,code) → 同一張假被存成 '事假'+'personal'
--   兩列時雙算(詹健如 事假 顯示 39.5h 可用,卻擋「已使用 28.2 天」)。
-- 修:改吃 hours(避開 days 髒值)+ 只認假別名稱(不雙算 name/code),與 LIFF 餘額同基準。
--   例:事假 type='事假' 實用 72.5h ≤ 上限 112h(14天×8) → 放行。
-- 抓 live 定義只改事件上限那段;其餘與線上逐字相同。
-- ════════════════════════════════════════════════════════════════════════════
