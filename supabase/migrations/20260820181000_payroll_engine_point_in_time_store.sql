-- Plan B Phase2:_compute_payroll_for_employee 門市改 point-in-time。
-- 載入 v_emp 後依計薪月份查 position_history 當月有效那列覆寫 v_emp.store/store_id,下游(政策獎金/天災/輸出快照)全對;查不到保留現況。preview/generate 同引擎。
...
BEGIN
  ...
  v_pit_store_id := NULL;
  SELECT ph.store_id INTO v_pit_store_id
