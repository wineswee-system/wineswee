-- Plan B Phase B2:底薪也 point-in-time(降職金額正確生效 + 歷史月保護)。
-- 引擎:歷史月(position_history 已結束區間)用當時底薪;當前月用 live salary_structures。
-- 轉調生效(trigger 立即 + cron 到日)同步更新 salary_structures.base_salary,當前月才反映新底薪。
-- 驗證:高承揚(ss暫設48000)8月=41000/微風、9月=48000/信義。
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(CASE WHEN lr.type IN ('事假','personal','無薪假','unpaid') THEN COALESCE(lr.hours, sn.h, COALESCE(lr.days,0)*8) ELSE 0 END),0),

    COALESCE(SUM(CASE WHEN lr.type IN ('事假','personal','無薪假','unpaid') THEN COALESCE(lr.days,0) ELSE 0 END),0),
  ...
  IF NOT v_is_hourly THEN
    ...
    v_sal_ratio  := CASE WHEN v_total_days > 0 THEN v_sal_actual::numeric / v_total_days ELSE 1 END;
  ...
  RETURN jsonb_build_object(
    ...
    '_leave_rows', COALESCE((
      ...
      SELECT jsonb_agg(jsonb_build_object('date', start_date, 'type', type, 'hours', COALESCE(hours, (SELECT SUM(public._scheduled_net_hours(p_emp_id, g::date)) FROM generate_series(start_date, COALESCE(end_date, start_date), interval '1 day') g), COALESCE(days,0)*8), 'days', days) ORDER BY start_date)
...
BEGIN
  IF NEW.status = '已核准' AND (OLD.status IS DISTINCT FROM '已核准') THEN
    ...
    IF NEW.effective_date <= CURRENT_DATE THEN
      ...
      -- 同步真理源:當前月底薪反映新值(歷史月靠 payroll point-in-time 保護)
      UPDATE public.salary_structures SET base_salary = NEW.new_base_salary
       WHERE employee_id = NEW.employee_id AND NEW.new_base_salary IS NOT NULL;
    END IF;
  END IF;
  RETURN NEW;
END $function$
...
BEGIN
  ...
  LOOP
    ...
    UPDATE public.salary_structures SET base_salary = r.base_salary
     WHERE employee_id = r.employee_id AND r.base_salary IS NOT NULL;
    n := n + 1;
  END LOOP;
  RETURN n;
END $function$
