-- 二代健保費率改讀 nhi_supplement_params(不再寫死 0.0211),費率單一來源永不飄。
-- 兩支引擎函式各加 v_nhi_rate(查 effective_year<=年度 的最新 rate,fallback 0.0211),取代內嵌 0.0211。
-- 現值 rate=0.0211 → 計算數字不變(已3人試算驗證 gross 一致)。
...
BEGIN
  ...
  IF NOT v_is_hourly THEN
    ...
    SELECT COUNT(DISTINCT g::date)::int INTO v_loa_days

      FROM public.leave_requests lr,

           generate_series(GREATEST(lr.start_date, v_eff_start), LEAST(lr.end_date, v_eff_end), interval '1 day') g
...
AS $function$
DECLARE
  v_nhi_rate numeric := COALESCE((SELECT rate FROM public.nhi_supplement_params WHERE effective_year <= p_year ORDER BY effective_year DESC LIMIT 1), 0.0211);
  v_run_id   INT;
  v_count    INT := 0;
    ...
    BEGIN
      ...
      IF rec.health_ins_grade IS NOT NULL THEN
        ...
        IF v_insured_salary IS NOT NULL AND v_insured_salary > 0 THEN
          ...
          IF v_taxable > 0 THEN
            v_premium := ROUND(v_taxable * v_nhi_rate);
