-- 留職停薪納入批次計薪:①scope 加 留停(generate_payroll/preview_payroll)②引擎 v_loa_days 也讀 leave_of_absence_requests(已核准)
-- 效果:留停會出現、本薪0(只扣落在當期月份的留停天數→留停前月份照領全額)、雇主勞健保照算。
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- ── 副作用:到期特休/2025結算/舊補休結算 折現「標記已折清」(引擎已把金額算進 gross;此處標記防下月重付)──
      --   離職/編制外由函式自身回0不動;現無到期→無副作用。補休(comp_time_ledger)標記亦一併接回(原孤兒沒接線)。
      PERFORM public._settle_expired_leave_cashout(rec.employee_id, v_run_id, v_month_end, false);
      PERFORM public._settle_expired_comp_time(rec.employee_id, v_run_id, v_month_end);
    ...
    overtime_pay_weekday, overtime_pay_restday, overtime_pay_holiday, overtime_pay_national, ot_hours_restday, ot_hours_national,
        ...
        prorate_ratio, actual_work_days, leave_breakdown
    ...
    (v_j->>'otPayWeekday')::numeric, (v_j->>'otPayRestday')::numeric, (v_j->>'otPayWeeklyOff')::numeric, (v_j->>'otPayHoliday')::numeric, (v_j->>'otRestday')::numeric, (v_j->>'otHoliday')::numeric,
        ...
        COALESCE(v_j->'_leave_rows','[]'::jsonb)
