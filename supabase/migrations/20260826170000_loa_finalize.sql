-- 留職停薪收尾:①引擎退保→保費全0(續保則雇主照付)②核准設留停加起始日閘門③process_effective_loa cron(到期自動留停/回任)
...
BEGIN
  ...
  END IF;
  v_gross := v_eff_base + v_ot_ovt_for_net + v_policy_bonus + v_unused_payout + v_sev_total + v_exp_leave;
    ...
    BEGIN
      IF NEW.status = '已核准' AND (OLD.status IS DISTINCT FROM '已核准') THEN
        IF NEW.start_date <= CURRENT_DATE THEN
          UPDATE employees SET status = '留停' WHERE id = NEW.employee_id AND status = '在職';
        END IF;  -- 未到起始日 → 由 process_effective_loa cron 到期自動設留停
      END IF;
      IF NEW.actual_return_date IS NOT NULL AND OLD.actual_return_date IS NULL THEN
        UPDATE employees SET status = '在職' WHERE id = NEW.employee_id AND status = '留停';
      END IF;
      RETURN NEW;
    END $function$
;

CREATE OR REPLACE FUNCTION public.process_effective_loa()
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
    ...
    BEGIN
      WITH upd AS (
        UPDATE employees e SET status='留停'
          FROM leave_of_absence_requests loa
         WHERE loa.employee_id=e.id AND loa.status='已核准'
           AND loa.start_date <= CURRENT_DATE
           AND (loa.actual_return_date IS NULL OR loa.actual_return_date > CURRENT_DATE)
           AND e.status='在職'
        RETURNING 1)
      SELECT count(*) INTO n FROM upd;
      -- 回任:實際回任日到了、還在留停 → 設回在職
      UPDATE employees e SET status='在職'
        FROM leave_of_absence_requests loa
       WHERE loa.employee_id=e.id AND loa.status='已核准'
         AND loa.actual_return_date IS NOT NULL AND loa.actual_return_date <= CURRENT_DATE
         AND e.status='留停';
      RETURN n;
    END $function$
;
...
-- cron(若無則建;每天 00:07)
SELECT cron.schedule('process-effective-loa','7 0 * * *','SELECT public.process_effective_loa()') WHERE NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname='process-effective-loa');
