-- PT(時薪)有薪假給付:特休/補休「先排班→請假」視同出席,按 leave_request 淨時數 × 時薪計入 gross。
-- 月薪不受影響(引擎只在 v_is_hourly 跑此段);base 已排除已轉假日→不重複給付。
-- 僅新增 4 處(2 變數宣告 + 1 計算區塊 + 併入 v_gross + json 欄位),其餘與線上版逐字相同。
...
BEGIN
  ...
  END IF;
  v_gross := v_eff_base + v_pt_paid_leave_pay + v_ot_ovt_for_net + v_policy_bonus + v_unused_payout + v_sev_total + v_exp_leave;
