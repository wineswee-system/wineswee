-- 全天假存的時數改抓班表(變形工時一天≠8h):create_leave_request + liff_insert_leave_request
-- 有排班→該假期間各日排定淨工時加總;無排班(辦公室)→ fallback 名目 v_hours(天數×8)。
-- 額度檢查一律用名目 v_hours(特休按天/8h);補休(時數制)改用 v_store_hours 保持檢查+扣帳一致。
-- 只影響新建請假單;歷史不動。實測:變形10h班日全天特休存10h、8h班日存8h、額度都扣1天。
