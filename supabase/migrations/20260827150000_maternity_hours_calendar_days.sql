-- 產假(曆日制)時數修正:hours = 曆日 × 8(對齊 days,不分假日)。原本 hours=工作日×8→與56天天數不符、餘額誤顯示可再請。
-- create_leave_request + liff_insert_leave_request 產假段各加 v_hours := v_days*8。含既有 #484 訂正(304→448)。
-- (本檔為兩支函式最新完整版,含前一版全天假抓班表變更 + 本次產假時數修正)
...
BEGIN
  ...
  IF p_unit = 'day' AND p_type_code IS DISTINCT FROM 'maternity' THEN
    SELECT SUM(public._scheduled_net_hours(v_emp.id, g::date)) INTO v_store_hours
...
BEGIN
  ...
  IF v_unit = 'day' AND v_code IS DISTINCT FROM 'maternity' THEN
    SELECT SUM(public._scheduled_net_hours(emp.id, g::date)) INTO v_store_hours
...
-- 訂正既有待審產假單時數
UPDATE public.leave_requests SET hours = days*8 WHERE type='產假' AND deleted_at IS NULL AND hours IS DISTINCT FROM days*8;
