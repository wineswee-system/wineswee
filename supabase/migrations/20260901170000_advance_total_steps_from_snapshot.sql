-- 修:推進/結案時「總關數」讀現行鏈 → 鏈就地改(如拿掉督導)後,舊單提早結案跳過快照最後幾關。
-- web_advance_chain_request、liff_approve_request 改成:有快照用快照關數,沒快照才用現行鏈。
-- (hr_chain_approve / expense_*_step_advance / form_submission_chain_approve 早已以快照為準)
...
BEGIN
  ...
  IF v_chain_id IS NOT NULL THEN
    ...
    -- ★ 總關數:有快照用快照關數(舊單凍的關數),沒快照才用現行鏈 → 避免鏈就地改後提早結案跳過關
    IF v_has_snapshot THEN
      SELECT COUNT(*) INTO v_total_steps FROM public.request_chain_snapshots
       WHERE request_type = v_snap_rt AND request_id = p_id;
    ELSE
    ...
    EXECUTE format('UPDATE %I SET current_step=$1, approved_by=$2 WHERE id=$3', v_table_name) USING v_effective_step, emp.name, p_id;
...
BEGIN
  ...
  IF p_type = 'expense_request' THEN
    ...
    IF EXISTS (SELECT 1 FROM public.request_chain_snapshots WHERE request_type='expense_request' AND request_id=p_id) THEN
      SELECT COUNT(*) INTO v_total_steps FROM public.request_chain_snapshots WHERE request_type='expense_request' AND request_id=p_id;
    ELSE
    ...
    IF v_is_last THEN
      ...
      RETURN json_build_object('ok', true, 'status','已核准', 'event','approved', 'is_last_step', true,
        'applicant', json_build_object('emp_id',
          ...
          'name', v_er.employee));
    ELSE
      UPDATE expense_requests SET current_step=current_step+1, approved_by=emp.name WHERE id=p_id;  -- ★逐關記簽核人(對齊 WEB expense_request_step_advance)→ASH 中間關才正確
      SELECT * INTO v_next_step FROM approval_chain_steps
