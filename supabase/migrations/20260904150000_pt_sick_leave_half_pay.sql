-- PT(時薪制)病假/未滿6月年資產假:補回法定半薪(勞工請假規則§4、勞基法§50)。
-- 舊洞:時薪本薪 = 時薪 × 實際出勤工時(v_hours 已跳過請假日)→ 病假那天本薪=0;
--   而 v_half_deduct 對時薪制=0(那條是給月薪從滿月薪扣半用),PT 有薪假補回又只認特休/補休,
--   三者合起來 → PT 請病假變成「0 元」,少給了法定半薪。
-- 修:對 v_is_hourly 新增 v_pt_half_leave_pay = ceil(v_half_hours × 時薪 × 0.5) 併入 gross。
--   v_half_hours 已含病假+未滿6月產假(§50同半薪桶)。月薪制 v_pt_half_leave_pay=0,邏輯完全不變。
-- 驗證:時薪420(220/h)8h病假→ptHalfLeavePay=880、halfPayDeduction=0、gross 0→880;
--       月薪108不變(ptHalfLeavePay=0、仍走halfPayDeduction=766)。preview/generate 共用同引擎一起生效。
  ...
  IF COALESCE(v_emp.in_payroll, false) = true AND NOT v_is_hourly THEN
    ...
    WHERE (CASE WHEN COALESCE(v_emp_category,'') = 'admin'
      ...
      AND NOT EXISTS (SELECT 1 FROM attendance_records ar
                       ...
                       WHERE ar.employee_id = p_emp_id AND ar.date = g.d::date AND ar.clock_in IS NOT NULL)
  ...
  RETURN jsonb_build_object(
    ...
    '_awol_rows', COALESCE((
      ...
      WHERE COALESCE(v_emp.in_payroll, false) = true AND NOT v_is_hourly
        ...
        AND NOT EXISTS (SELECT 1 FROM attendance_records ar WHERE ar.employee_id = p_emp_id AND ar.date = g.d::date AND ar.clock_in IS NOT NULL)
