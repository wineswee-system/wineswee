-- 門市權責清理:舊 user_stores(2026-04 灌檔)大量是「本店員工自己」的冗餘列(store_id=本店),
-- 不是真正「額外指派」。修正:
--   1) 「額外可存取」只顯示真正跨店的人(ue.store_id <> s.id)→ 頁面不再被本店員工洗版。
--   2) 「可排班」改用正確判定:有 schedule.edit 且對該店有範圍(_can_see_store_for_emp:本店/負責人/督導/user_stores)
--      → 涵蓋本店儲幹/店長、負責人、督導、跨店額外,不靠冗餘列。
