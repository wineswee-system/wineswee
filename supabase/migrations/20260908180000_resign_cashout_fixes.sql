-- 離職折現兩修:
--  1) 特休折現基數 = 底薪+津貼(role/meal/transport),分母固定 30(原只用底薪、分母用當月天數)
--  2) 補休(comp_time_ledger)離職時一次結清所有 active(不只過期 expires_at<月底)
-- 引擎 _compute_payroll_for_employee(preview/generate 共用);離職者需重跑該月薪資。
