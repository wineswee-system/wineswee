-- 本薪只認「有排班日的班表整點時數」;有打卡但對不到排班(休息日/加班日出勤)不再灌進本薪、也不吃打卡分鐘
-- 修正:郭沂倫/陳泳蓁/游詩婷 等在排班「休息」日上班,打卡時數被算進本薪→本薪非220倍數;休息日出勤應走加班單
-- 只改 _compute_payroll_for_employee 內 eff_hours 的兩處 total_hours fallback → 0(遲到/早退照舊只走單次扣,不動本薪)
...
BEGIN
  ...
  IF v_pit_store_id IS NOT NULL THEN
    v_emp.store_id := v_pit_store_id;
    SELECT name INTO v_emp.store FROM public.stores WHERE id = v_pit_store_id;
  END IF;
  ...
  -- 缺一隻打卡(有排班、非請假)→ 讀班表算工時(整天用班表);否則(對不到排班)本薪不計→走加班單
  ...
  FROM (
    ...
    SELECT
      ...
      CASE
        ...
        WHEN ar.clock_in IS NOT NULL AND ar.clock_out IS NOT NULL THEN CASE WHEN s.actual_start IS NOT NULL AND s.actual_end IS NOT NULL THEN public._shift_seg_hours(s.actual_start, s.actual_end) ELSE 0 /* 對不到排班→本薪不計,休息日/加班日出勤走加班單 */ END
        ...
        ELSE 0 /* 對不到排班→本薪不計,休息日/加班日出勤走加班單 */
