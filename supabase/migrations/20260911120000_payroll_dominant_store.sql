-- 計薪門市歸屬改「當月主門市」:出勤天數多者→平手比排班時數→再平手歸新店 — 2026-09-11
-- 新增 helper _dominant_store_for_month;引擎只換門市判定(base 的 PIT 邏輯不動)。
-- preview_payroll / generate_payroll 共用此引擎 → 兩邊同時生效。

CREATE OR REPLACE FUNCTION public._dominant_store_for_month(p_emp_id int, p_mstart date, p_mend date)
RETURNS int LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $fn$
DECLARE v_cnt int; v_store int;
BEGIN
  SELECT count(DISTINCT store_id) INTO v_cnt FROM public.position_history
   WHERE employee_id=p_emp_id AND store_id IS NOT NULL
     AND effective_date <= p_mend AND (end_date IS NULL OR end_date >= p_mstart);
  IF v_cnt = 0 THEN RETURN NULL; END IF;
  IF v_cnt = 1 THEN
    SELECT store_id INTO v_store FROM public.position_history
     WHERE employee_id=p_emp_id AND store_id IS NOT NULL
       AND effective_date <= p_mend AND (end_date IS NULL OR end_date >= p_mstart)
  ...
  END IF;
  WITH segs AS (
    SELECT store_id, max(effective_date) AS eff FROM public.position_history
     WHERE employee_id=p_emp_id AND store_id IS NOT NULL
       AND effective_date <= p_mend AND (end_date IS NULL OR end_date >= p_mstart)
     GROUP BY store_id),
  att AS (
    SELECT ar.store_id, count(DISTINCT ar.date) AS days FROM public.attendance_records ar
     WHERE ar.employee_id=p_emp_id AND ar.clock_in IS NOT NULL
       AND ar.date BETWEEN p_mstart AND p_mend GROUP BY ar.store_id),
  sch AS (
    SELECT ph.store_id, SUM(COALESCE(s.actual_hours,0)+COALESCE(s.actual_hours_2,0)) AS hrs
      FROM public.schedules s
      JOIN public.position_history ph ON ph.employee_id=p_emp_id AND ph.store_id IS NOT NULL
       AND s.date >= ph.effective_date AND s.date <= COALESCE(ph.end_date, p_mend)
     WHERE s.employee_id=p_emp_id AND s.date BETWEEN p_mstart AND p_mend
     GROUP BY ph.store_id)
  SELECT g.store_id INTO v_store FROM segs g
    LEFT JOIN att ON att.store_id=g.store_id
    LEFT JOIN sch ON sch.store_id=g.store_id
   ORDER BY COALESCE(att.days,0) DESC, COALESCE(sch.hrs,0) DESC, g.eff DESC LIMIT 1;
  RETURN v_store;
END $fn$;
REVOKE ALL ON FUNCTION public._dominant_store_for_month(int,date,date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public._dominant_store_for_month(int,date,date) TO authenticated;
...
DECLARE
  ...
  v_nhi_rate       numeric := COALESCE((SELECT rate FROM public.nhi_supplement_params WHERE effective_year <= split_part(p_period,'-',1)::int ORDER BY effective_year DESC LIMIT 1), 0.0211);  -- 二代健保費率:讀表,不再寫死
  ...
  v_store_id       int;
  v_pit_store_id   int;   -- point-in-time 門市(依計薪月份回推)
  v_pit_base       numeric;
  v_pit_hist       boolean;
  v_dom_store      int;
  ...
  v_pt_paid_leave_hours numeric := 0;   -- PT(時薪)有薪假(特休/補休)折算工時
  v_pt_paid_leave_pay   numeric := 0;   -- = v_pt_paid_leave_hours × 時薪
  v_pt_half_leave_pay   numeric := 0;   -- PT(時薪)病假/未滿6月產假半薪 = v_half_hours × 時薪 × 0.5

  v_absence_days   numeric:=0;

  v_awol_days      numeric:=0;   -- 曠職天數（編制內行政:應上班卻整天沒打卡且沒請假/出差）

  v_awol_deduct    numeric:=0;   -- 曠職扣款\
  ...
  v_sev_amount     numeric := 0;

  v_sev_notice     numeric := 0;

  v_sev_total      numeric := 0;
...
BEGIN
  ...
  -- ★ point-in-time 門市:依計薪月份回推 position_history 當月有效那列;查不到才保留現況
  v_pit_store_id := NULL; v_pit_base := NULL; v_pit_hist := false;
  SELECT ph.store_id, ph.base_salary, (ph.end_date IS NOT NULL) INTO v_pit_store_id, v_pit_base, v_pit_hist
    FROM public.position_history ph
   WHERE ph.employee_id = p_emp_id
     AND ph.effective_date <= v_mend
     AND (ph.end_date IS NULL OR ph.end_date >= v_mstart)
   ORDER BY ph.effective_date DESC LIMIT 1;
  v_dom_store := public._dominant_store_for_month(p_emp_id, v_mstart, v_mend);
  IF v_dom_store IS NOT NULL THEN
    v_emp.store_id := v_dom_store;
    SELECT name INTO v_emp.store FROM public.stores WHERE id = v_dom_store;
  END IF;
  ...
  -- 員工身分工時規則(取代讀門市):依 salary_structures.employment_category 讀
  SELECT
    EXTRACT(EPOCH FROM ecwr.work_start::time)/60.0,
    EXTRACT(EPOCH FROM ecwr.work_end::time)/60.0,
    ecwr.grace_minutes
  INTO v_office_start_min, v_office_end_min, v_admin_grace
  FROM public.employment_category_work_rules ecwr
  WHERE ecwr.organization_id = v_emp.organization_id
    AND ecwr.category = COALESCE(v_emp_category,'') AND ecwr.is_active;

  v_admin_grace := COALESCE(v_admin_grace, CASE WHEN COALESCE(v_emp_category,'')='admin' THEN 30 ELSE 0 END);  -- 查無門市/NULL → 30；門市設 0 則保留 0
  ...
  -- 缺一隻打卡(有排班、非請假)→ 讀班表算工時(整天用班表);否則用實際 total_hours

  SELECT

    COALESCE(SUM(eff_hours),0),

    COALESCE(SUM(eff_hours) FILTER (WHERE is_natl),0),

    COALESCE(SUM(late_minutes) FILTER (WHERE is_late AND late_minutes > tol),0),
  ...
  FROM (
    ...
    SELECT
      ...
      CASE
        ...
        WHEN ar.clock_in IS NOT NULL AND ar.clock_out IS NOT NULL THEN CASE WHEN s.actual_start IS NOT NULL AND s.actual_end IS NOT NULL THEN public._shift_seg_hours(s.actual_start, s.actual_end) ELSE COALESCE(ar.total_hours,0) END

        WHEN (ar.clock_in IS NULL) <> (ar.clock_out IS NULL)
             ...
             AND NOT EXISTS (SELECT 1 FROM leave_requests lr
                             ...
                             WHERE lr.employee_id = ar.employee_id AND lr.status = '已核准' AND lr.deleted_at IS NULL

                               AND ar.date BETWEEN lr.start_date AND COALESCE(lr.end_date, lr.start_date))

          THEN public._shift_seg_hours(s.actual_start, s.actual_end)

        ELSE COALESCE(ar.total_hours,0)

      END AS eff_hours,

      public._is_national_holiday(ar.employee_id, ar.date) AS is_natl,

      ar.is_late, ar.late_minutes,

      COALESCE(NULLIF(st.late_tolerance_minutes,0),5) AS tol
  ...
  --   統一邏輯：有班表(actual_start/end)就用班表；admin 沒班表才退回固定 09:00–18:00(應下班 clamp(clock_in+9h, v_end_base−grace, v_end_base+grace)=±30 雙向:早進早走、晚進晚走)。
  ...
  IF NOT v_is_piece THEN
    ...
    FROM (
      ...
      SELECT
        ...
        EXTRACT(EPOCH FROM ar.clock_in::time)/60.0 + CASE WHEN s.actual_start IS NOT NULL AND s.actual_end IS NOT NULL AND EXTRACT(EPOCH FROM s.actual_end::time) <= EXTRACT(EPOCH FROM s.actual_start::time) AND EXTRACT(EPOCH FROM ar.clock_in::time) < 21600 THEN 1440 ELSE 0 END AS ci,

        (EXTRACT(EPOCH FROM ar.clock_out::time)/60.0)

          + CASE WHEN ar.clock_out IS NOT NULL

                  AND ( EXTRACT(EPOCH FROM ar.clock_out::time) < EXTRACT(EPOCH FROM ar.clock_in::time) OR ( s.actual_start IS NOT NULL AND s.actual_end IS NOT NULL AND EXTRACT(EPOCH FROM s.actual_end::time) <= EXTRACT(EPOCH FROM s.actual_start::time) AND EXTRACT(EPOCH FROM ar.clock_out::time) < 21600 ) )
        ...
        COALESCE(EXTRACT(EPOCH FROM s.actual_end::time)/60.0,
                 ...
                 CASE WHEN COALESCE(v_emp_category,'')='admin'
                      ...
                      THEN LEAST(GREATEST(EXTRACT(EPOCH FROM ar.clock_in::time)/60.0 + v_span, v_end_base - v_admin_grace), v_end_base + v_admin_grace)

                      ELSE NULL END) AS ae,

        v_admin_grace AS grace,
      ...
      WHERE ar.employee_id = p_emp_id AND ar.date >= v_effd_start AND ar.date <= v_effd_end
        ...
        -- 缺下班卡且有班表 → 走班表 fallback,該日整天用班表,不算遲到早退

        AND NOT (ar.clock_out IS NULL AND s.actual_start IS NOT NULL AND s.actual_end IS NOT NULL)
  ...
  ELSE
    ...
    v_base_salary := CASE WHEN v_pit_hist AND v_pit_base IS NOT NULL THEN v_pit_base ELSE COALESCE(v_ss.base_salary, v_emp.base_salary, 0) END;  -- 歷史月用當時底薪,當前月用真理源
  ...
  FROM (
    ...
    SELECT ot_hours, is_exception,
      ...
      public._ot_category(p_emp_id, (CASE WHEN start_time < TIME '06:00' THEN request_date - 1 ELSE request_date END), ot_category) AS cat

    FROM overtime_requests

    WHERE employee_id = p_emp_id AND status='已核准' AND NOT COALESCE(is_comp_leave,false) AND deleted_at IS NULL
  ...
  FROM (
    ...
    FROM (
      ...
      SELECT (CASE WHEN start_time < TIME '06:00' THEN request_date - 1 ELSE request_date END) AS request_date, ot_hours, COALESCE(is_exception,false) AS is_exc,

        public._ot_category(p_emp_id, (CASE WHEN start_time < TIME '06:00' THEN request_date - 1 ELSE request_date END), ot_category) cat
  ...
  WHERE employee_id=p_emp_id AND status='active' AND (expires_at < v_mend OR (v_emp.status = '離職' AND v_emp.resign_date IS NOT NULL AND v_emp.resign_date BETWEEN v_mstart AND v_mend));
  ...
  SELECT
    ...
    COALESCE(SUM(CASE WHEN lr.type IN ('事假','personal','無薪假','unpaid','天災假','天災','disaster') THEN COALESCE(lr.hours, sn.h, COALESCE(lr.days,0)*8) ELSE 0 END),0),

    COALESCE(SUM(CASE WHEN lr.type IN ('事假','personal','無薪假','unpaid','天災假','天災','disaster') THEN COALESCE(lr.days,0) ELSE 0 END),0),

    COALESCE(SUM(CASE
      WHEN lr.type IN ('病假','sick','生理假','menstrual') THEN COALESCE(lr.hours, sn.h, COALESCE(lr.days,0)*8)
      -- 產假:未滿6個月年資→半薪(§50,用病假半薪機制);滿6個月→全薪(不進半薪、不扣)
      WHEN lr.type IN ('產假','maternity')
           AND v_emp.join_date IS NOT NULL
           AND lr.start_date < (v_emp.join_date::date + interval '6 months')
        THEN COALESCE(lr.hours, sn.h, COALESCE(lr.days,0)*8)
      ELSE 0 END),0)
  ...
  LEFT JOIN LATERAL (
    ...
    SELECT SUM(public._scheduled_net_hours(p_emp_id, g::date)) AS h

    FROM generate_series(lr.start_date, COALESCE(lr.end_date, lr.start_date), interval '1 day') g

  ) sn ON true

  WHERE lr.employee_id=p_emp_id AND lr.status='已核准'

    AND lr.deleted_at IS NULL

    AND lr.start_date >= v_mstart AND lr.start_date <= v_mend;

  v_absence_days := v_unpaid_days;

  -- ── PT(時薪)有薪假:特休/補休「先排班→請假」視同出席,按核准請假單淨時數×時薪給付 ──
  --   月薪本來就照給、不適用;只 hourly。時數優先 lr.hours(時段假=實際淨工時/全天=8×天),
  --   fallback 該日排定淨工時;跟引擎既有請假時數同一套 COALESCE。type=特休/補休(含 annual/comp 別名)。
  IF v_is_hourly THEN
    SELECT COALESCE(SUM(COALESCE(lr.hours, sn.h, COALESCE(lr.days,0)*8)), 0)
    INTO v_pt_paid_leave_hours
    FROM leave_requests lr
    LEFT JOIN LATERAL (
      SELECT SUM(public._scheduled_net_hours(p_emp_id, g::date)) AS h
      FROM generate_series(lr.start_date, COALESCE(lr.end_date, lr.start_date), interval '1 day') g
    ) sn ON true
    WHERE lr.employee_id = p_emp_id AND lr.status='已核准' AND lr.deleted_at IS NULL
      AND lr.type IN ('特休','annual','特別休假','補休','comp')
      AND lr.start_date >= v_mstart AND lr.start_date <= v_mend;
    v_pt_paid_leave_pay := ceil(v_pt_paid_leave_hours * v_hourly_rate);
    -- ★ PT(時薪)病假半薪:本薪已跳過請假日(那天=0),依法(勞工請假規則§4)補回時薪×時數×0.5。
    --   v_half_hours 已含病假 + 未滿6月年資產假(§50同走半薪桶)。月薪制走 v_half_deduct 扣半,不進這條。
    v_pt_half_leave_pay := ceil(v_half_hours * v_hourly_rate * 0.5);
  END IF;



  -- ── 曠職（編制內行政:應上班日整天沒打卡、且沒核准請假/出差）──

  --   應上班日 = 週一~五 且 非國定假(holidays.is_workday=false) 且 非天災日(disaster_days,依門市)。

  --   忘打卡但有上班 → 補打卡核准後產生 attendance_records → 不算曠職。任職期邊界已由 v_effd_* 收斂。

  IF COALESCE(v_emp.in_payroll, false) = true AND NOT v_is_hourly THEN

    SELECT COUNT(*)::numeric INTO v_awol_days

    FROM generate_series(v_effd_start::timestamp, v_effd_end::timestamp, interval '1 day') g(d)

    WHERE (CASE WHEN COALESCE(v_emp_category,'') = 'admin'

                THEN EXTRACT(DOW FROM g.d) NOT IN (0,6)

                     AND NOT EXISTS (SELECT 1 FROM holidays hh WHERE hh.date = g.d::date AND hh.is_workday = false)

                ELSE EXISTS (SELECT 1 FROM schedules s

                              WHERE s.employee_id = p_emp_id AND s.date = g.d::date
      ...
      AND NOT EXISTS (SELECT 1 FROM disaster_days dd

                       WHERE dd.date = g.d::date AND dd.organization_id = v_emp.organization_id

                         AND (dd.store_ids IS NULL OR v_emp.store_id = ANY(dd.store_ids)))

      AND NOT EXISTS (SELECT 1 FROM attendance_records ar

                       WHERE ar.employee_id = p_emp_id AND ar.date = g.d::date AND (ar.clock_in IS NOT NULL OR ar.clock_out IS NOT NULL))

      AND NOT EXISTS (SELECT 1 FROM leave_requests lr

                       WHERE lr.employee_id = p_emp_id AND lr.status = '已核准' AND lr.deleted_at IS NULL

                         AND g.d::date BETWEEN lr.start_date AND COALESCE(lr.end_date, lr.start_date))

      AND NOT EXISTS (SELECT 1 FROM business_trips bt

                       WHERE bt.employee = v_emp.name AND bt.status = '已核准'

                         AND g.d::date BETWEEN bt.start_date AND COALESCE(bt.end_date, bt.start_date));
  ...
  v_awol_deduct    := CASE WHEN v_is_hourly THEN 0 ELSE floor(v_awol_days * 8 * v_hourly_rate) END;  -- 曠職:每天 8h × 時薪 = base_for_ins/30

  v_absence_deduct := v_absence_deduct + v_awol_deduct;

  v_absence_days   := v_absence_days + v_awol_days;
  ...
  IF NOT v_is_hourly THEN
    ...
    -- ★留職停薪(育嬰留停):扣掉被已核准留停假覆蓋的在職天數 → 底薪+津貼按剩餘天數比例(整月留停→0)。

    --   保費 proration 走 v_in_service(在職)不受影響 → 公司負擔勞健保照付;整月留停 gross<=0 由後段夾成 實領0+員工自付0。

    SELECT COUNT(DISTINCT g::date)::int INTO v_loa_days FROM (
      SELECT gs AS g FROM public.leave_requests lr,
        generate_series(GREATEST(lr.start_date, v_eff_start), LEAST(lr.end_date, v_eff_end), interval '1 day') gs
       WHERE lr.employee_id = p_emp_id AND lr.status = '已核准' AND lr.deleted_at IS NULL
         AND lr.type IN ('育嬰留職停薪','留職停薪','育嬰假')
         AND lr.start_date <= v_eff_end AND lr.end_date >= v_eff_start
      UNION
      SELECT gs AS g FROM public.leave_of_absence_requests loa,
        generate_series(GREATEST(loa.start_date, v_eff_start), LEAST(COALESCE(loa.actual_return_date - 1, loa.planned_end_date, v_eff_end), v_eff_end), interval '1 day') gs
       WHERE loa.employee_id = p_emp_id AND loa.status = '已核准'
         AND loa.start_date <= v_eff_end
         AND COALESCE(loa.actual_return_date - 1, loa.planned_end_date, v_eff_end) >= v_eff_start
    ) x;

    v_sal_actual := GREATEST(v_sal_actual - COALESCE(v_loa_days, 0), 0);

    v_sal_ratio  := CASE
      WHEN v_total_days <= 0 THEN 1
  ...
  v_is_final := (v_emp.status = '離職' AND v_resign IS NOT NULL AND EXTRACT(YEAR FROM v_resign)=v_year AND EXTRACT(MONTH FROM v_resign)=v_month);

  IF v_is_final AND v_join IS NOT NULL AND (v_join + INTERVAL '6 months')::date <= COALESCE(v_resign, current_date) THEN
    ...
    IF NOT v_is_hourly THEN
      ...
      v_unused_payout := ceil(v_unused_days * ((v_base_salary + COALESCE(v_role_allow,0) + COALESCE(v_meal,0) + COALESCE(v_transport,0)) / 30.0));
  ...
  -- ── B3: 資遣金入當月薪資（離職當月抓 severance_records:資遣金+預告工資加進 gross。特休折現已由 B2 v_unused_payout 計,不重複加）──

  IF v_is_final THEN

    SELECT sr.id, COALESCE(sr.severance_amount,0), COALESCE(sr.notice_wage,0)

      INTO v_sev_rec_id, v_sev_amount, v_sev_notice

    FROM severance_records sr

    WHERE sr.employee_id = p_emp_id

      AND sr.termination_date >= v_mstart AND sr.termination_date <= v_mend

      AND COALESCE(sr.status,'') NOT IN ('cancelled','rejected','已取消','已退回','void')

    ORDER BY sr.id DESC

    LIMIT 1;

    v_sev_total := COALESCE(v_sev_amount,0) + COALESCE(v_sev_notice,0);

  END IF;



  -- 到期未休特休/2025結算/舊補休結算 每月自動折現(離職走v_unused_payout、編制外由函式擋→回0)。
  -- read-only(dry_run=true)只算金額進gross;真入帳的「標記used_days防重付」在 generate_payroll 做。
  IF NOT v_is_final THEN
    v_exp_leave := public._settle_expired_leave_cashout(p_emp_id, NULL, v_mend, true);
  END IF;
  v_gross := v_eff_base + v_pt_paid_leave_pay + v_pt_half_leave_pay + v_ot_ovt_for_net + v_policy_bonus + v_unused_payout + v_sev_total + v_exp_leave;
  ...
  -- 健保按「月底最後一日在保」計收:當月最後一天前已離職(退保) → 當月健保保費歸0(員工+雇主)。
  --   不同於勞保按日 prorate;不影響二代健保補充保費(依實際給付另計)。
  IF v_resign IS NOT NULL AND v_resign < v_mend THEN
    v_health_emp := 0;
    v_health_er  := 0;
  END IF;
  ...
  IF v_health_insured > 0 THEN
    ...
    IF v_overtime_pay > v_health_insured THEN
      ...
      v_nhi_supp := v_nhi_supp + floor((v_overtime_pay - v_health_insured) * v_nhi_rate);

      v_nhi_breakdown := v_nhi_breakdown || jsonb_build_object(

        'category','加班費超額','income',v_overtime_pay,'exempt',v_health_insured,

        'taxable',v_overtime_pay - v_health_insured,'rate',v_nhi_rate,

        'premium',floor((v_overtime_pay - v_health_insured) * v_nhi_rate));
    ...
    BEGIN
      ...
      IF v_bonus > 0 THEN
        ...
        IF v_taxb > 0 THEN
          ...
          v_nhi_supp := v_nhi_supp + floor(v_taxb * v_nhi_rate);

          v_nhi_breakdown := v_nhi_breakdown || jsonb_build_object(

            'category','高額獎金累計','income',v_bonus,'cumulative',v_newc,

            'threshold_4x',v_4x,'taxable',v_taxb,'rate',v_nhi_rate,'premium',floor(v_taxb*v_nhi_rate));
  ...
  v_wage_grade  := public._pension_bracket_row(v_year, COALESCE(NULLIF(v_emp.labor_pension_grade, 0), v_eff_base));  -- 勞退提繳工資：有填固定級距用它、空的才薪資推算
  ...
  -- 薪資為 0（整月未出勤/無底薪）→ 員工自付勞健保/自提歸 0，實領不倒扣成負；

  --   雇主負擔(v_labor_er/v_health_er/v_pension_er)照舊,公司該付仍付。

  IF v_gross <= 0 THEN

    v_labor_emp := 0; v_health_emp := 0; v_pension_self := 0;
  ...
  -- 退保留停:整月被「退保」的已核准 LoA 覆蓋 → 勞健保/勞退全不計(員工+雇主皆0)
  IF EXISTS (SELECT 1 FROM public.leave_of_absence_requests loa
              WHERE loa.employee_id = p_emp_id AND loa.status = '已核准'
                AND COALESCE(loa.keep_insurance, true) = false
                AND loa.start_date <= v_mstart
                AND COALESCE(loa.actual_return_date - 1, loa.planned_end_date, v_mend) >= v_mend) THEN
    v_labor_emp := 0; v_labor_er := 0;
    v_health_emp := 0; v_health_er := 0;
    v_pension_self := 0; v_pension_er := 0;
    v_total_deduct := 0;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'ptPaidLeaveHours', v_pt_paid_leave_hours, 'ptPaidLeavePay', v_pt_paid_leave_pay,
    'ptHalfLeavePay', v_pt_half_leave_pay,
    ...
    'awolDays', v_awol_days,

    'awolDeduction', v_awol_deduct,
    ...
    'expired_leave_cashout', v_exp_leave,

    'unused_leave_days', v_unused_days,

    'is_final_settlement', v_is_final,

    'severance_amount', v_sev_amount,

    'severance_notice_wage', v_sev_notice,

    'severance_total', v_sev_total,

    'severance_record_id', v_sev_rec_id,
    ...
    '_ot_rows', COALESCE((
      ...
      FROM (
        ...
        SELECT request_date, cat, SUM(ot_hours) AS reg_h FROM (
          ...
          SELECT (CASE WHEN start_time < TIME '06:00' THEN request_date - 1 ELSE request_date END) AS request_date, ot_hours, public._ot_category(p_emp_id, (CASE WHEN start_time < TIME '06:00' THEN request_date - 1 ELSE request_date END), ot_category) cat

          FROM overtime_requests WHERE employee_id=p_emp_id AND status='已核准' AND NOT COALESCE(is_comp_leave,false) AND deleted_at IS NULL AND NOT COALESCE(is_exception,false)
    ...
    '_ot_exception_rows', COALESCE((
      ...
      FROM (
        ...
        FROM (
          ...
          SELECT (CASE WHEN start_time < TIME '06:00' THEN request_date - 1 ELSE request_date END) AS request_date, ot_hours, COALESCE(is_exception,false) is_exc, public._ot_category(p_emp_id, (CASE WHEN start_time < TIME '06:00' THEN request_date - 1 ELSE request_date END), ot_category) cat

          FROM overtime_requests WHERE employee_id=p_emp_id AND status='已核准' AND NOT COALESCE(is_comp_leave,false) AND deleted_at IS NULL
    ...
    '_leave_rows', COALESCE((
      ...
      SELECT jsonb_agg(jsonb_build_object('date', start_date, 'end_date', end_date, 'type', type, 'hours', COALESCE(hours, (SELECT SUM(public._scheduled_net_hours(p_emp_id, g::date)) FROM generate_series(start_date, COALESCE(end_date, start_date), interval '1 day') g), COALESCE(days,0)*8), 'days', days) ORDER BY start_date)
    ...
    '_awol_rows', COALESCE((
      ...
      SELECT jsonb_agg(g.d::date ORDER BY g.d)

      FROM generate_series(v_effd_start::timestamp, v_effd_end::timestamp, interval '1 day') g(d)

      WHERE COALESCE(v_emp.in_payroll, false) = true AND NOT v_is_hourly

        AND (CASE WHEN COALESCE(v_emp_category,'') = 'admin'

                  THEN EXTRACT(DOW FROM g.d) NOT IN (0,6)

                       AND NOT EXISTS (SELECT 1 FROM holidays hh WHERE hh.date = g.d::date AND hh.is_workday = false)

                  ELSE EXISTS (SELECT 1 FROM schedules s WHERE s.employee_id = p_emp_id AND s.date = g.d::date

                                AND s.actual_start IS NOT NULL AND COALESCE(s.shift,'') NOT IN ('休','休息','例假','補休'))

             END)

        AND NOT EXISTS (SELECT 1 FROM disaster_days dd WHERE dd.date = g.d::date AND dd.organization_id = v_emp.organization_id AND (dd.store_ids IS NULL OR v_emp.store_id = ANY(dd.store_ids)))

        AND NOT EXISTS (SELECT 1 FROM attendance_records ar WHERE ar.employee_id = p_emp_id AND ar.date = g.d::date AND (ar.clock_in IS NOT NULL OR ar.clock_out IS NOT NULL))

        AND NOT EXISTS (SELECT 1 FROM leave_requests lr WHERE lr.employee_id = p_emp_id AND lr.status = '已核准' AND lr.deleted_at IS NULL AND g.d::date BETWEEN lr.start_date AND COALESCE(lr.end_date, lr.start_date))

        AND NOT EXISTS (SELECT 1 FROM business_trips bt WHERE bt.employee = v_emp.name AND bt.status = '已核准' AND g.d::date BETWEEN bt.start_date AND COALESCE(bt.end_date, bt.start_date))
      ...
      FROM (
        ...
        FROM (
          ...
          WHERE ar.employee_id=p_emp_id AND ar.date>=v_effd_start AND ar.date<=v_effd_end AND ar.clock_in IS NOT NULL AND NOT v_is_piece
            AND NOT (ar.clock_out IS NULL AND s.actual_start IS NOT NULL AND s.actual_end IS NOT NULL)  -- 對齊扣款:缺下班卡且有班表→不算遲到/早退(避免顯示假遲到)

            AND NOT (COALESCE(hh.is_workday = false, false) OR (COALESCE(v_emp_category,'') = 'admin' AND EXTRACT(DOW FROM ar.date) IN (0,6)))

            AND NOT EXISTS (SELECT 1 FROM leave_requests lr WHERE lr.employee_id=ar.employee_id AND lr.status='已核准' AND lr.deleted_at IS NULL AND ar.date BETWEEN lr.start_date AND COALESCE(lr.end_date,lr.start_date))
      ...
      FROM (
        ...
        FROM (
          ...
          SELECT ar.date AS dt,
            ...
            (EXTRACT(EPOCH FROM ar.clock_out::time)/60.0) + CASE WHEN ar.clock_out IS NOT NULL AND (EXTRACT(EPOCH FROM ar.clock_out::time)<EXTRACT(EPOCH FROM ar.clock_in::time) OR (s.actual_start IS NOT NULL AND s.actual_end IS NOT NULL AND EXTRACT(EPOCH FROM s.actual_end::time) <= EXTRACT(EPOCH FROM s.actual_start::time) AND EXTRACT(EPOCH FROM ar.clock_out::time)<21600)) THEN 1440 ELSE 0 END AS cot,

            COALESCE(EXTRACT(EPOCH FROM s.actual_start::time)/60.0, CASE WHEN COALESCE(v_emp_category,'')='admin' THEN v_start_base ELSE NULL END) AS ast,

            COALESCE(EXTRACT(EPOCH FROM s.actual_end::time)/60.0, CASE WHEN COALESCE(v_emp_category,'')='admin' THEN LEAST(GREATEST(EXTRACT(EPOCH FROM ar.clock_in::time)/60.0+v_span,v_end_base - v_admin_grace),v_end_base + v_admin_grace) ELSE NULL END) AS ae,
