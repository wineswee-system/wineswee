-- 請假天數(days)改吃實際班表:有排班→數當期實際工作天(對齊 hours 已吃班表的做法);無排班→保留 leave_calc 行事曆值
-- 修正變形工時/排到週末假日者 hours(班表)與 days(行事曆)不一致(如黃為燁 9/08~9/13 5天40h被算成4天)
-- 兩支建單:liff_insert_leave_request + create_leave_request。產假(曆日制)不受影響。
...
DECLARE
  ...
  v_days numeric;
  v_hours NUMERIC;
  v_store_hours NUMERIC;
  v_sched_days numeric;
  v_deduct JSON;
BEGIN
  ...
  -- 全天假存的時數改抓班表(變形工時一天≠8h):有排班→各日排定淨工時加總;無排班→名目 v_hours。
  -- ★ 額度檢查一律用名目 v_hours 不動;補休(時數制)改用 v_store_hours 保持檢查+扣帳一致。
  v_store_hours := v_hours;
  IF v_unit = 'day' AND v_code IS DISTINCT FROM 'maternity' THEN
    SELECT SUM(public._scheduled_net_hours(emp.id, g::date)),
           COUNT(*) FILTER (WHERE public._scheduled_net_hours(emp.id, g::date) > 0)
      INTO v_store_hours, v_sched_days
      FROM generate_series((p_payload->>'start_date')::date,
             COALESCE((p_payload->>'end_date')::date, (p_payload->>'start_date')::date), interval '1 day') g;
    v_store_hours := COALESCE(v_store_hours, v_hours);
    -- ★ 天數也吃班表:有排班→數實際工作天(對齊 hours);無排班→保留 leave_calc 行事曆值
    IF COALESCE(v_sched_days,0) > 0 THEN v_days := v_sched_days; END IF;
  END IF;
  ...
  IF v_code = 'maternity' THEN
    ...
    v_hours := v_days * 8;   -- 產假曆日制:時數不分假日=曆日×8(對齊天數)
  END IF;
  ...
  IF v_type = '補休' THEN
    ...
    BEGIN
      SELECT COALESCE(SUM(hours - hours_used), 0) INTO v_avail
       ...
       WHERE employee_id = emp.id AND status = 'active' AND expires_at >= CURRENT_DATE;
      IF v_avail < v_store_hours THEN
        RAISE EXCEPTION '補休餘額不足：剩 % 小時，需請 % 小時', v_avail, v_store_hours;
      END IF;
    END;
  END IF;
  ...
  IF v_code IS NOT NULL AND v_code NOT IN ('annual','comp_time','maternity') THEN
    ...
    BEGIN
      ...
      IF v_max IS NOT NULL AND v_max > 0 THEN
        -- 用小時比對 + 只認「假別名稱」(對齊 LIFF 餘額顯示):
        --   ① days 欄有髒值(時數假常被寫成 days=時數) → 改吃 hours;
        --   ② 同一張假若同時存成 name('事假') 與 code('personal') 兩列會被 IN 雙算 → 只認 name。
        SELECT COALESCE(SUM(COALESCE(hours, days*8)),0) INTO v_used
          FROM public.leave_requests
         WHERE employee_id = emp.id
           AND type = COALESCE(v_ltname, v_type)
           AND status NOT IN ('已拒絕','已退回','已駁回','已取消')
           AND start_date >= v_ystart;
        IF v_used + v_hours > v_max*8 + 0.001 THEN
          RAISE EXCEPTION '%已使用 % 小時，年度上限 % 小時，剩餘 % 小時，不足申請 % 小時（含待審核）',
            COALESCE(v_ltname, v_type), round(v_used,1), v_max*8,
            round(greatest(0, v_max*8 - v_used),1), round(v_hours,1);
        END IF;
      END IF;
    END;
  END IF;
  ...
  -- ★ 結算桶(特休假2025結算/舊人資系統補休結算等,可休可折現):送出當下擋單筆超額。
  --   used_days 已含已核准(trg_settlement_leave_used_sync 維護),再扣同型「待審核/審核中」在飛單。
  IF v_type LIKE '%結算%' THEN
    DECLARE
      v_s_total numeric; v_s_used numeric; v_s_pend numeric; v_s_remain numeric;
    BEGIN
      SELECT total_days, used_days INTO v_s_total, v_s_used
        FROM public.leave_balances
       WHERE employee_id = emp.id AND leave_type = v_type
         AND (period_start IS NULL OR period_start <= (p_payload->>'start_date')::date)
         AND (expires_at   IS NULL OR expires_at   >= (p_payload->>'start_date')::date)
       ORDER BY year DESC LIMIT 1;
      IF v_s_total IS NOT NULL THEN
        SELECT COALESCE(SUM(days),0) INTO v_s_pend
          FROM public.leave_requests
         WHERE employee_id = emp.id AND type = v_type
           AND status NOT IN ('已拒絕','已退回','已駁回','已取消','已核准')
           AND deleted_at IS NULL;
        v_s_remain := v_s_total - COALESCE(v_s_used,0) - COALESCE(v_s_pend,0);
        IF v_days > v_s_remain + 0.001 THEN
          RAISE EXCEPTION '%餘額不足:剩 % 小時,待審核 % 小時,本次申請 % 小時',
            v_type, round(greatest(0,v_s_remain)*8,1), round(COALESCE(v_s_pend,0)*8,1), round(v_days*8,1);
        END IF;
      END IF;
    END;
  END IF;
  ...
  IF v_type = '補休' THEN
    v_deduct := public.deduct_comp_time(new_id, emp.id, v_store_hours);
...
BEGIN
  ...
  IF p_type_code = 'maternity' THEN
    ...
    v_hours := v_days * 8;   -- 產假曆日制:時數不分假日=曆日×8(對齊天數;§50半薪扣款也吃此值)
  END IF;
  ...
  -- 全天假存的時數改抓班表(變形工時一天≠8h):有排班→各日排定淨工時加總;無排班→名目 v_hours。
  -- ★ 額度檢查一律用名目 v_hours 不動(特休按天/8h);補休(時數制)改用 v_store_hours 保持檢查+扣帳一致。
  v_store_hours := v_hours;
  IF p_unit = 'day' AND p_type_code IS DISTINCT FROM 'maternity' THEN
    SELECT SUM(public._scheduled_net_hours(v_emp.id, g::date)),
           COUNT(*) FILTER (WHERE public._scheduled_net_hours(v_emp.id, g::date) > 0)
      INTO v_store_hours, v_sched_days
  ...
  IF p_type_code = 'comp_time' THEN
    ...
    IF v_comp_bal < v_store_hours THEN
      RAISE EXCEPTION '補休餘額不足:剩 % 小時,本次要請 % 小時', v_comp_bal, v_store_hours;
    END IF;
  END IF;
  ...
  IF p_type_code = 'annual' THEN
    ...
    ELSE
      ...
      END IF;
    END IF;
  -- ── 謀職假(§16):限資遣預告期在職者;每星期不超過二日之工作時間(=16h) ──
  ELSIF p_type_code = 'job_seeking' THEN
    IF NOT public._employee_job_seeking_eligible(v_emp.id) THEN
      RAISE EXCEPTION '謀職假限資遣預告期間員工申請';
    END IF;
    IF (SELECT COALESCE(SUM(COALESCE(hours, days*8)),0) FROM public.leave_requests
         WHERE employee_id = v_emp.id AND type IN ('謀職假','job_seeking')
           AND status NOT IN ('已拒絕','已退回','已駁回','已取消')
           AND date_trunc('week', start_date) = date_trunc('week', p_start_date)
       ) + v_hours > 16 + 0.001 THEN
      RAISE EXCEPTION '謀職假每星期上限二日(16小時):本週(含本次)已超過,請分散到其他週';
    END IF;
  ...
  INSERT INTO public.leave_requests (
    ...
    v_days, v_store_hours, p_reason, '待審核', v_emp.organization_id,
