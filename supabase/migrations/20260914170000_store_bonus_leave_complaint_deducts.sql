-- 營運獎金辦法 Phase2d:病假/事假/特休/客訴 做成可設定金額的個人扣;督導業績定額=0(只拿1份管理獎金)
ALTER TABLE public.store_bonus_role_config
  ADD COLUMN IF NOT EXISTS sick_leave_deduct numeric(10,2) DEFAULT 100,
  ADD COLUMN IF NOT EXISTS personal_leave_deduct numeric(10,2) DEFAULT 200,
  ADD COLUMN IF NOT EXISTS annual_leave_deduct numeric(10,2) DEFAULT 200,
  ADD COLUMN IF NOT EXISTS complaint_deduct numeric(10,2) DEFAULT 500;
ALTER TABLE public.store_bonus_employee
  ADD COLUMN IF NOT EXISTS sick_leave_count int DEFAULT 0,
  ADD COLUMN IF NOT EXISTS personal_leave_count int DEFAULT 0,
  ADD COLUMN IF NOT EXISTS annual_leave_count int DEFAULT 0,
  ADD COLUMN IF NOT EXISTS complaint_count int DEFAULT 0;
UPDATE public.store_bonus_role_config SET target_bonus_amount=0 WHERE role='督導';
