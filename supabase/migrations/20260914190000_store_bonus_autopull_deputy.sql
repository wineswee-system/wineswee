...
BEGIN
  ...
  -- 依組織圖帶入該店督導 = 店長(manager_id)的直屬主管,且職稱含督導
  DECLARE v_dep_id INT; v_dep_name TEXT; v_dep_w NUMERIC;
  BEGIN
    SELECT e2.id, e2.name INTO v_dep_id, v_dep_name
      FROM stores s JOIN employees e1 ON e1.id = s.manager_id
                    JOIN employees e2 ON e2.id = e1.supervisor_id
     WHERE s.id = p_store_id AND e2.status = '在職' AND COALESCE(e2.position,'') ILIKE '%督導%';
    IF v_dep_id IS NOT NULL THEN
      SELECT weight INTO v_dep_w FROM store_bonus_role_config WHERE organization_id = v_org_id AND role = '督導';
      INSERT INTO store_bonus_employee (monthly_id, employee_id, employee_name, role, weight)
      VALUES (v_monthly_id, v_dep_id, v_dep_name, '督導', COALESCE(v_dep_w,1))
      ON CONFLICT (monthly_id, employee_id) DO UPDATE SET role='督導', weight=EXCLUDED.weight, employee_name=EXCLUDED.employee_name;
    END IF;
  END;
