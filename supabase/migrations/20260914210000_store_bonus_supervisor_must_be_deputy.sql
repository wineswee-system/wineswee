-- 修:開單拉『manager 的主管』只在該主管職稱為督導時才拉(避免拉到執行長等高層)
    ...
    SELECT e.id, e.name
     ...
     WHERE e.organization_id = v_org_id
       ...
       AND (
         ...
         OR e.id = (SELECT e1.supervisor_id FROM stores s JOIN employees e1 ON e1.id = s.manager_id JOIN employees e2 ON e2.id = e1.supervisor_id WHERE s.id = p_store_id AND COALESCE(e2.position,'') ILIKE '%督導%')
       )
     ORDER BY e.id
  LOOP
