-- ④ 計時全勤激勵:LIFF 薪資袋加 parttime_incentives(兼職於發放月看到全勤加給)
...
BEGIN
  ...
  RETURN json_build_object(
    ...
    'parttime_incentives', (
      SELECT COALESCE(json_agg(json_build_object(
        'year_month', pt.year_month, 'paid_hours', pt.paid_hours,
        'rate_add', pt.rate_add, 'bonus', pt.bonus, 'payout_year_month', pt.payout_year_month
      ) ORDER BY pt.year_month DESC), '[]'::json)
      FROM public.part_time_attendance_incentive pt
      WHERE pt.employee_id = emp.id AND COALESCE(pt.bonus,0) > 0
    )
  );
END $function$
;
