...
import { ALL, getProduct, related, CATEGORY_LABEL, CATEGORIES } from '../../lib/products'
...
export function generateMetadata({ params }: { params: { id: string } }): Metadata {
  ...
  return {
    title: p.name,
    description: `${p.name}${p.price ? ` — NT$${p.price.toLocaleString()}` : ''}｜Wineswee 威士威酒食超市`,
...
export default function ProductPage({ params }: { params: { id: string } }) {
  ...
  const catLabel = CATEGORY_LABEL[p.cat] || '嚴選'
  const catEn = CATEGORIES.find(c => c.key === p.cat)?.en || 'Selection'
  ...
  return (
    <>
      ...
      <nav className="crumb wrap">
        <Link href="/">首頁</Link><span>/</span>
        <Link href={`/category/${p.cat}`}>{catLabel}</Link><span>/</span>
        <em>{p.name}</em>
      </nav>
      ...
      <section className="pdp wrap">
        <div className="pdp-media">
          {p.image ? (
            ...
            <span className="pdp-ph">Wineswee</span>
          )}
        ...
        <div className="pdp-info">
          ...
          <div className="pdp-price">
            {p.price ? <><span className="u">NT$</span>{p.price.toLocaleString()}</> : <span className="ask">價格洽詢</span>}
          </div>
      ...
      {rel.length > 0 && (
        <ProductRow no="＋" id={`cat-${p.cat}`} title={`更多${catLabel}`} en={catEn} items={rel} tone="cream" />
      )}
